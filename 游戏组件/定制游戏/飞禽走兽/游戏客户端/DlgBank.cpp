...
void CDlgBank::OnBnClickedOk()
{
  ...
  if(false==m_bBankStorage && (m_lInCount <= 0 || m_lInCount > m_lStorageGold))
  {
    ...
    if(m_lStorageGold>0)
    {
      strMessage.Format(TEXT("你输入的游戏币值必须在1和%I64d之间"), m_lStorageGold);
    }
    else
    {
  ...
  if(true==m_bBankStorage && (m_lInCount <= 0 || m_lInCount > m_lGameGold))
  {
    ...
    if(m_lGameGold>0)
    {
      strMessage.Format(TEXT("你输入的游戏币值必须在1和%I64d之间"),m_lGameGold);
    }
    else
    {
      strMessage.Format(TEXT("你的当前游戏币数目为0,不能进行存储操作!"));
    }
    MessageBox(strMessage,TEXT("温馨提示"));
