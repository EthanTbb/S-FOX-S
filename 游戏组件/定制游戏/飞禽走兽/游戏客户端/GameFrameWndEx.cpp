...
//屏幕限制
#define LESS_SCREEN_CY        750                 //最小高度
#define LESS_SCREEN_CX        1246                //最小宽度

//////////////////////////////////////////////////////////////////////////////////

BEGIN_MESSAGE_MAP(CGameFrameWndEx, CFrameWnd)
...
//构造函数
CGameFrameWndEx::CGameFrameWndEx()
{
  //滚动变量
  m_nScrollXPos=0L;
  m_nScrollYPos=0L;
...
//接口查询
VOID * CGameFrameWndEx::QueryInterface(REFGUID Guid, DWORD dwQueryVer)
{
...
//消息解释
BOOL CGameFrameWndEx::PreTranslateMessage(MSG * pMsg)
{
  //提示消息
...
//命令函数
BOOL CGameFrameWndEx::OnCommand(WPARAM wParam, LPARAM lParam)
{
  //变量定义
  ...
  switch(nCommandID)
  {
    ...
    case IDC_MUTE:
    {
      ...
      //设置按钮
      HINSTANCE hInstance=AfxGetInstanceHandle();//GetModuleHandle(GAME_FRAME_DLL_NAME);
      m_btMuteContrl.SetButtonImage((pGlobalUnits->m_bMuteStatuts==false)?IDB_BT_SLIENT:IDB_BT_MUTE,hInstance,true,false);
    ...
    case IDC_RULE:
    {
      ...
      {
        //创建对象
        if(m_pDlgGameRule==NULL)
        {
          m_pDlgGameRule=new CDlgGameRule;
        }
...
//还原窗口
bool CGameFrameWndEx::RestoreWindow()
{
  //状态判断
  if(m_bMaxShow==true)
  {
    ...
    //设置按钮
    m_btMax.SetButtonImage(IDB_BT_MAX,AfxGetInstanceHandle(),false,false);
...
//最大窗口
bool CGameFrameWndEx::MaxSizeWindow()
{
  //状态判断
  if(m_bMaxShow==false)
  {
    ...
    //设置按钮
    m_btMax.SetButtonImage(IDB_BT_RESORE,AfxGetInstanceHandle(),false,false);
...
//调整控件
VOID CGameFrameWndEx::RectifyControl(INT nWidth, INT nHeight)
{
  //状态判断
  ...
  if((m_nScrollXMax==0)||(m_nScrollYMax==0))
  {
    return;
  }
  ...
  //控制按钮
  DeferWindowPos(hDwp,m_btOption,NULL,m_nScrollXMax-59,2,0,0,uFlags|SWP_NOSIZE);
  ...
  DeferWindowPos(hDwp,m_btMin,NULL,m_nScrollXMax-87,2,0,0,uFlags|SWP_NOSIZE);
  DeferWindowPos(hDwp,m_btMax,NULL,m_nScrollXMax-59,2,0,0,uFlags|SWP_NOSIZE);
  DeferWindowPos(hDwp,m_btClose,NULL,m_nScrollXMax-31,2,0,0,uFlags|SWP_NOSIZE);
  ...
  if(pIGameFrameView!=NULL)
  {
    ...
    if(rcWnd.Width()<LESS_SCREEN_CX)
    {
      ...
      //增加游戏区域
      DeferWindowPos(hDwp,hWndView,NULL,EncircleInfoFrame.nLBorder,EncircleInfoFrame.nTBorder,rcWnd.Width()-EncircleInfoFrame.nRBorder-10,
...
//绘画背景
BOOL CGameFrameWndEx::OnEraseBkgnd(CDC * pDC)
{
  //获取标题
  CString strTitle;
...
//按键测试
UINT CGameFrameWndEx::OnNcHitTest(CPoint Point)
{
  //按钮测试
  if(m_bMaxShow==false)
  {
    ...
    {
      if(ClientPoint.y<=CAPTION_SIZE)
      {
        return HTTOPLEFT;
      }
      if(ClientPoint.y>=(rcClient.Height()-BORAD_SIZE))
      {
        return HTBOTTOMLEFT;
      }
      return HTLEFT;
...
//位置消息
VOID CGameFrameWndEx::OnSize(UINT nType, INT cx, INT cy)
{
...
//位置信息
VOID CGameFrameWndEx::OnGetMinMaxInfo(MINMAXINFO * lpMMI)
{
...
//建立消息
INT CGameFrameWndEx::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
  ...
  m_btClose.SetButtonImage(IDB_BT_EX_CLOSE,TEXT("BT_MAIN_CLOSE"),hInstance,false,false);
  m_btOption.SetButtonImage(IDB_BT_OPTION,hInstance,false,false);
  m_btMuteContrl.SetButtonImage((pGlobalUnits->m_bMuteStatuts==false)?IDB_BT_SLIENT:IDB_BT_MUTE,hInstance,false,false);
  m_btRule.SetButtonImage(IDB_BT_RULE,hInstance,false,false);

  //控制窗口
  AfxSetResourceHandle(GetModuleHandle(GAME_FRAME_DLL_NAME));
  m_GameFrameControl.Create(8011,this);
  //m_GameFrameControl.ShowWindow(SW_HIDE);
  AfxSetResourceHandle(GetModuleHandle(NULL));
...
//鼠标消息
VOID CGameFrameWndEx::OnLButtonDown(UINT nFlags, CPoint Point)
{
...
//鼠标消息
VOID CGameFrameWndEx::OnLButtonDblClk(UINT nFlags, CPoint Point)
{
...
//设置改变
VOID CGameFrameWndEx::OnSettingChange(UINT uFlags, LPCTSTR lpszSection)
{
...
//标题消息
LRESULT CGameFrameWndEx::OnSetTextMesage(WPARAM wParam, LPARAM lParam)
{
  //默认调用
...
//游戏规则
bool CGameFrameWndEx::ShowGameRule()
{
//  try
//  {
//    //创建对象
...
//声音控制
bool CGameFrameWndEx::AllowGameSound(bool bAllowSound)
{
  //设置变量
  CGlobalUnits *pGlobalUnits=CGlobalUnits::GetInstance();
...
//控制接口
bool CGameFrameWndEx::OnGameOptionChange()
{
  //定义变量
  CGlobalUnits *pGlobalUnits=CGlobalUnits::GetInstance();
...
//播放完成
VOID CGameFrameWndEx::OnEventFlashFinish()
{
  m_FlashControl.ShowControl(SW_HIDE);
}

//道具成功
VOID  CGameFrameWndEx::OnPropertySuccess(LPCTSTR szSourceNickName,DWORD dwSourceUserID,DWORD dwTargerUserID,WORD wPropertyIndex,WORD wPropertyCount)
{
  //获取对象
...
//道具失败
VOID CGameFrameWndEx::OnPropertyFailure(LONG lErrorCode,LPCTSTR szDescribeString)
{
  //弹出消息
  CInformation Information;
...
//旁观控制
bool CGameFrameWndEx::AllowGameLookon(DWORD dwUserID, bool bAllowLookon)
{
  //发送消息
...
//窗口销毁
void CGameFrameWndEx::OnDestroy()
{
  __super::OnDestroy();
