...
void CGameRecord::FillGameRecourd(tagGameRecord & GameRecord)
{
  ...
  //游戏局数
  myprintf(szBuffer,CountArray(szBuffer),TEXT("%d"),GameRecord.wDrawCount);
  INT nItemIndex=m_RecordGameList.InsertItem(m_RecordGameList.GetItemCount(),szBuffer);

  //游戏赢家
  if(GameRecord.cbWinerSide==0)
  {
    m_RecordGameList.SetItem(nItemIndex,wColumnCount++,LVIF_TEXT,TEXT("闲家"),0,0,0,0);
  }
  else if(GameRecord.cbWinerSide==1)
  {
    m_RecordGameList.SetItem(nItemIndex,wColumnCount++,LVIF_TEXT,TEXT("平家"),0,0,0,0);
  }
  else
  {
    m_RecordGameList.SetItem(nItemIndex,wColumnCount++,LVIF_TEXT,TEXT("庄家"),0,0,0,0);
  }

  //本局成绩
  myprintf(szBuffer,CountArray(szBuffer),TEXT("%I64d"),GameRecord.lGameScore);
  m_RecordGameList.SetItem(nItemIndex,wColumnCount++,LVIF_TEXT,szBuffer,0,0,0,0);

  //本局下注
  myprintf(szBuffer,CountArray(szBuffer),TEXT("%I64d"),GameRecord.lMyAddGold);
  m_RecordGameList.SetItem(nItemIndex,wColumnCount++,LVIF_TEXT,szBuffer,0,0,0,0);

  //闲家下注
  myprintf(szBuffer,CountArray(szBuffer),TEXT("%I64d"),GameRecord.lDrawPlayerScore);
  m_RecordGameList.SetItem(nItemIndex,wColumnCount++,LVIF_TEXT,szBuffer,0,0,0,0);

  //平家下注
  myprintf(szBuffer,CountArray(szBuffer),TEXT("%I64d"),GameRecord.lDrawTieScore);
  m_RecordGameList.SetItem(nItemIndex,wColumnCount++,LVIF_TEXT,szBuffer,0,0,0,0);

  //庄家下注
  myprintf(szBuffer,CountArray(szBuffer),TEXT("%I64d"),GameRecord.lDrawBankerScore);
  m_RecordGameList.SetItem(nItemIndex,wColumnCount++,LVIF_TEXT,szBuffer,0,0,0,0);

  //闲家牌点
  myprintf(szBuffer,CountArray(szBuffer),TEXT("%d"),GameRecord.cbPlayerPoint);
  m_RecordGameList.SetItem(nItemIndex,wColumnCount++,LVIF_TEXT,szBuffer,0,0,0,0);

  //庄家牌点
  myprintf(szBuffer,CountArray(szBuffer),TEXT("%d"),GameRecord.cbBankerPoint);
  m_RecordGameList.SetItem(nItemIndex,wColumnCount++,LVIF_TEXT,szBuffer,0,0,0,0);
