...
//静态变量
CD3DTexture					CCardControl::m_TexCard;						//图片资源
...
// 		if (m_TexCard.IsNull()) 
// 			m_TexCard.LoadImage(hResInstance,(m_bSmallMode==false)?IDB_GAME_CARD:IDB_GAME_CARD);
// 	
...
//加载资源
void CCardControl::LoadGameResource(CD3DDevice * pD3DDevice, INT nWidth, INT nHeight)
{
	//加载资源
	HINSTANCE hResInstance=AfxGetInstanceHandle();
	if (m_TexCard.IsNull()) 
		m_TexCard.LoadImage(pD3DDevice,hResInstance,TEXT("PNG_GAME_CARD"),TEXT("PNG"));

	m_TexOpenCardHandle.LoadImage(pD3DDevice,hResInstance,TEXT("PNG_HAND"),TEXT("PNG"));
	//m_TexCardSelect.LoadImage(pD3DDevice,hResInstance,TEXT("PNG_CARD_SELECT"),TEXT("PNG"));

	//获取大小
	m_CardSize.SetSize(m_TexCard.GetWidth()/13,m_TexCard.GetHeight()/5);
}

//析构函数
CCardControl::~CCardControl()
{
	//if (m_TexCard.IsNull()==false) m_TexCard.Destory();
}
...
bool CCardControl::SetCardData(const BYTE cbCardData[], WORD wCardCount)
{
	...
	//效验参数
	WORD wCount = (WORD)m_CardTempItemArray.GetCount();
	ASSERT(wCardCount<=wCount);
	if (wCardCount>wCount) return false;
...
// 	}
	...
	for (WORD i=0;i<m_wCardCount;i++)
	{
		...
		if(m_blGameEnd)
		{
			m_TexCard.DrawImage(pD3DDevice,OriginPoint.x+nXDrawPos+15,OriginPoint.y+nYDrawPos,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos);
		}else
		{
				//绘画扑克
				if(1==i)
				{
					m_TexCard.DrawImage(pD3DDevice,OriginPoint.x+nXDrawPos+m_MovePoint.x+25,OriginPoint.y+nYDrawPos+m_MovePoint.y,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos);
					//没花纹好搓了，搓字吧
					//m_TexOpenCardHandle.DrawImage(pD3DDevice,OriginPoint.x-5,OriginPoint.y+nYDrawPos-5,m_TexOpenCardHandle.GetWidth(),m_TexOpenCardHandle.GetHeight(),0,0,RGB(255,0,255));
	
				}
				else
					m_TexCard.DrawImage(pD3DDevice,OriginPoint.x+nXDrawPos+25,OriginPoint.y+nYDrawPos,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos);
		...
		if ((m_wStartIndex<=i)&&(i<=m_wEndIndex))
		{
			...
			//m_TexCardSelect.DrawImage(pD3DDevice,OriginPoint.x+nXDrawPos,OriginPoint.y+nYDrawPos,m_TexCardSelect.GetWidth(),m_TexCardSelect.GetHeight(),0,0);
		}
	}
...
void CCardControl::OnMouseMove(UINT nFlags, CPoint wndPoint)
{
	...
	if(m_dwCurrentIndex>=1&&m_dwCurrentIndex !=INVALID_ITEM)
	{
		...
		if(nXPos>m_CardSize.cx)
		{
			nXPos = m_CardSize.cx;		
		}

		if(nYPos>m_CardSize.cy)
		{
			nYPos = m_CardSize.cy;
		}
		m_MovePoint.y =  nYPos;
