...
//时间标识
#define IDI_BANK_OPERATE_GET		101									//银行定时
#define IDI_BANK_OPERATE_SAVE		102									//银行定时

#define IDI_CHECK_BANKER			103									//检查上庄
#define IDI_REQUEST_BANKER			104									//申请定时
#define IDI_GIVEUP_BANKER			105									//下庄定时
...
bool  CAndroidUserItemSink::OnEventTimer(UINT nTimerID)
{
	...
	case IDI_BANK_OPERATE_GET:	//银行操作
		{
			m_pIAndroidUserItem->KillGameTimer(nTimerID);
	...
	case IDI_BANK_OPERATE_SAVE:	//银行操作
		{
			m_pIAndroidUserItem->KillGameTimer(nTimerID);
	...
	default:
		{
			if (nTimerID >= IDI_PLACE_JETTON && nTimerID <= IDI_PLACE_JETTON+MAX_CHIP_TIME)
			{
				...
				LONGLONG lMaxChipLmt = __min(lScore, m_lMaxChipUser);			//最大可下注值
				//统计次数
				m_nChipTimeCount++;
...
bool  CAndroidUserItemSink::OnEventGameMessage(WORD wSubCmdID, VOID * pBuffer, WORD wDataSize)
{
	...
	case SUB_S_WAIT_BANKER:			//等待庄家
		{
			return true;
		}
	case SUB_S_ADMIN_COMMDN:		//控制
		{
			return true;
		}
