...
//设置窗口
void CCardControl::SetSinkWindow(CGameFrameViewGDI * pSinkWindow)
{
	//设置变量
...
//弹起扑克
DWORD CCardControl::SetShootCard(BYTE bCardData[], DWORD dwCardCount)
{
	//变量定义
	...
	for (DWORD i=0;i<dwCardCount;i++)
	{
		for (INT_PTR j=0;j<m_CardDataItem.GetCount();j++)
		{
			...
			if ((pCardItem->bShoot==false)&&(pCardItem->bCardData==bCardData[i])) 
			{
				bChangeStatus=true;
...
void CCardControl::OnRButtonUp(UINT nFlags, CPoint Point)
{
	...
	//发送消息
	if (m_pSinkWindow!=NULL) m_pSinkWindow->PostEngineMessage(IDM_RIGHT_HIT_CARD,0,0);
...
void CCardControl::OnLButtonUp(UINT nFlags, CPoint Point)
{
	...
	//发送消息
	if (m_pSinkWindow!=NULL) m_pSinkWindow->PostEngineMessage(IDM_LEFT_HIT_CARD,0,0);
