...
#define IDI_START_GAME				111			                        //开始时间
//////////////////////////////////////////////////////////////////////////
	...
	case IDI_BANK_OPERATE:		//银行操作
		{
			...
			LONGLONG lRobotScore = pUserItem->GetUserScore();			
			{
				...
				ASSERT(pAndroidParameter->lMaxTakeScore>pAndroidParameter->lMinTakeScore);
				if(pAndroidParameter==NULL) return true;
//				if(pAndroidParameter->lMaxTakeScore<=pAndroidParameter->lMinTakeScore) return true;
					
                //存分操作
//				LONGLONG lAreaScore = pAndroidParameter->lMaxTakeScore-pAndroidParameter->lMinTakeScore;
				if(lRobotScore>pAndroidParameter->lMaxTakeScore)
				{
					//取得保存分
					LONGLONG lSaveScore = 0,lOutScore=0,lRandGetScore = 0,lSaveAeraScore = 0;
					lOutScore = lRobotScore - pAndroidParameter->lMaxTakeScore;

					//取小概率杂分
					if(rand()%4==0)
					{
						lSaveAeraScore = lAreaScore+lOutScore;
						int nRand = rand()%99 +1;
						lRandGetScore = lSaveAeraScore*nRand/100;
						if(lRandGetScore==0) lRandGetScore = lSaveAeraScore/2;
						lSaveScore = lRandGetScore;
					}
					else //整数
					{
						int nRand = rand()%99 +1;
						lRandGetScore = lAreaScore*nRand/100;
						if(lRandGetScore==0) lRandGetScore = lAreaScore/2;
						lSaveScore = lOutScore + lRandGetScore;
					}
				...
				else if (lRobotScore<pAndroidParameter->lMinTakeScore)
				{
					//提分
					LONGLONG lTakeScore = 0,lDisScore=0,lRandGetScore = 0,lTakeAeraScore=0;
					lDisScore = pAndroidParameter->lMinTakeScore - lRobotScore;

					//取小概率杂分
					if(rand()%4==0)
					{
                       lTakeAeraScore = lDisScore + lAreaScore;
					   int nRand = rand()%99 +1;
					   lRandGetScore = lTakeAeraScore*nRand/100;
					   if(lRandGetScore==0) lRandGetScore = lTakeAeraScore/2;
					   lTakeScore = lRandGetScore;
					}
					else
					{
						int nRand = rand()%99 +1;
						lRandGetScore = lAreaScore*nRand/100;
						if(lRandGetScore==0) lRandGetScore = lAreaScore/2;
						lTakeScore = lDisScore + lRandGetScore;
					}
	...
	case GAME_STATUS_FREE:			//空闲状态
		{
			...
			memcpy(m_szRoomName, pStatusFree->szServerName, sizeof(m_szRoomName));
			...
			{
				UINT nElapse=rand()%(pStatusFree->cbTimeLeave-6)+5;
	...
	case GS_GAME_END:		//结束状态
		{
			...
			memcpy(m_szRoomName, pStatusPlay->szServerName, sizeof(m_szRoomName));
...
bool CAndroidUserItemSink::OnSubGameFree(const VOID * pBuffer, WORD wDataSize)
{
	...
	//银行操作
	if (pGameFree->cbTimeLeave > 3&&rand()%2==0)
		m_pIAndroidUserItem->SetGameTimer(IDI_BANK_OPERATE, rand() %5 + 4);

	bool bMeGiveUp = false;
	if (m_wCurrentBanker == m_pIAndroidUserItem->GetChairID())
	{
		m_nBankerCount++;
		if ( m_nBankerCount >= m_nRobotBankerCount&&rand()%3==0) //够条件后取30%的下庄机会
		{
			//机器人走庄
			m_nBankerCount = 0;
	...
	{
		UINT nElapse=rand()%(pGameFree->cbTimeLeave-6)+5;
		m_pIAndroidUserItem->SetGameTimer(IDI_START_GAME,nElapse);
	}
...
{	
	...
	for (int i = 0; i < m_nChipTime; i++)
	{
		...
		nElapse = 0 + (nTimeGrid*i)/1000 + ( (rand()+wMyID)%(nRandRage*2) - (nRandRage-1) );
