...
class FlashSink : public ShockwaveFlashObjects2::_IShockwaveFlashEvents2
{
public:	
    LPCONNECTIONPOINT		m_ConnectionPoint;	
    DWORD					m_dwCookie;
    int						m_nRefCount;
    FlashPlayer2*			m_pFlashWidget;
...
public:
    ...
    HRESULT Init(FlashPlayer2* theFlashWidget)
    {
        ...
        if ((m_pFlashWidget->m_pFlashInterface->QueryInterface(IID_IConnectionPointContainer, (void**) &aConnectionPoint) == S_OK) &&
            (aConnectionPoint->FindConnectionPoint(__uuidof(ShockwaveFlashObjects2::_IShockwaveFlashEvents2), &m_ConnectionPoint) == S_OK))			
        {
    ...
    HRESULT STDMETHODCALLTYPE QueryInterface(REFIID riid, LPVOID* ppv)
    {
        *ppv = NULL;
        ...
        else if (riid == __uuidof(ShockwaveFlashObjects2::_IShockwaveFlashEvents2))
        {
            *ppv = (ShockwaveFlashObjects2::_IShockwaveFlashEvents2*) this;
            AddRef();
            return S_OK;
        }
        else
...
public:
    ...
    void Init(FlashPlayer2* theFlashWidget)
    {
        m_pFlashWidget = theFlashWidget;
        m_pFlashWidget->m_nCOMCount++;
    }
    ...
    {	
        ...
        {
            m_pFlashWidget->m_bFlashDirty = TRUE;
            //CopyRect(&m_pFlashWidget->m_rcFlashDirty,pRect);
        }
        else
        {			
...
FlashPlayer2::FlashPlayer2()
{
    ...
    char	path[MAX_PATH];
    //sprintf(path, "%s\\Flash.ocx", g_GlobalUnits.GetWorkDirectory());
    sprintf(path, "G:\\0577\\运行\\release\\Flash.ocx");

    m_hFlashLibHandle = LoadLibraryA(path);
    ...
    m_pOleObject->QueryInterface(__uuidof(IShockwaveFlash2), (LPVOID*) &m_pFlashInterface);
...
FlashPlayer2::~FlashPlayer2()
{
    ...
    m_FlashCommands.clear();
}

double FlashPlayer2::GetFlashVersion()
{
    CoInitialize(NULL);
    ...
    IShockwaveFlash2* aFlashInterface = NULL;
    if (FAILED(anOleObject->QueryInterface(__uuidof(IShockwaveFlash2), (LPVOID*) &aFlashInterface)))
        return 0.0;
...
void FlashPlayer2::CleanupBuffer()
{
    if (m_hBitmap != NULL)
...
void FlashPlayer2::RebuildBuffer(LONG nWidth,LONG nHeight)
{
    CleanupBuffer();

    m_hFlashDC = ::CreateCompatibleDC(0);
    ...
    m_hBitmap = CreateDIBSection(m_hFlashDC, &m_BMPINFO, DIB_RGB_COLORS, &m_pFrameBuffer, NULL, 0);
...
void FlashPlayer2::GetFlashSize(LONG* nWidth,LONG* nHeight)
{
    *nWidth = m_nWidth;
    *nHeight = m_nHeight;
}

void FlashPlayer2::GetBitmapSize(LONG* nWidth,LONG* nHeight)
{
    *nWidth = m_BMPINFO.bmiHeader.biWidth;
    *nHeight = abs(m_BMPINFO.bmiHeader.biHeight);
}

HDC FlashPlayer2::GetFlashDC()
{
    return m_hFlashDC;
}

HBITMAP FlashPlayer2::GetFlashBitmap()
{
    return m_hBitmap;
}

BITMAPINFO FlashPlayer2::GetBitmapInfo()
{
    return m_BMPINFO;
}

void* FlashPlayer2::GetFlashFrameBuffer()
{
    return m_pFrameBuffer;
}


void FlashPlayer2::ReSetShowRect(int nw, int nh)
{
    m_nWidth = nw;
    m_nHeight = nh;
    ...
    RebuildBuffer(m_nWidth,m_nHeight);
...
bool FlashPlayer2::StartAnimation(const std::wstring& theFileName,LONG nWidth,LONG nHeight,HWND hWnd,LONG nBmpWidth,LONG nBmpHeight)
{
    std::wstring aFullPath = theFileName;

    _bstr_t bstr((TCHAR*) aFullPath.c_str());		
    m_pFlashInterface->put_Movie(bstr);

    //	if ((m_nPauseCount == 0) && (m_pFlashInterface->Play() != S_OK))
    //		return false;

    ReSetShowRect(nWidth, nHeight);
    m_FlashCommands.clear();
...
void FlashPlayer2::SetQuality(FLASHQUALITY theQuality)
{
...
bool FlashPlayer2::GetFlashCommand(std::string& sCommand, std::string& sParam)
{
    if (m_FlashCommands.empty()) return false;

    FLASHCOMMAND cmd = m_FlashCommands.front();
    m_FlashCommands.pop_front();

    sCommand = cmd.sCommand;
    sParam = cmd.sParam;
    return true;
}

void FlashPlayer2::FlashCommand(const std::string& sCommand, const std::string& sParam)
{
    FLASHCOMMAND cmd;
    cmd.sCommand = sCommand;
    cmd.sParam = sParam;

    m_FlashCommands.push_back(cmd);
}

bool FlashPlayer2::IsPlaying()
{
...
void FlashPlayer2::Stop()
{
    if (m_pFlashInterface != NULL)
    {
        VARIANT_BOOL isPlaying = 0;
        m_pFlashInterface->put_Playing(isPlaying);
    }
}

void FlashPlayer2::Pause(bool bShow)
{
    m_nPauseCount++;
...
void FlashPlayer2::GotoFrame(int theFrameNum)
{
    if (m_pFlashInterface != NULL)
    {
...
int FlashPlayer2::GetCurrentFrame()
{	
...
int FlashPlayer2::GetTotalFrames()
{
    long aTotalFrame = -1;
    if (m_pFlashInterface != NULL)
...
int FlashPlayer2::GetFrameNum()
{
    long aFrameNum = -1;
    if (m_pFlashInterface != NULL)
        m_pFlashInterface->get_FrameNum(&aFrameNum);
    return aFrameNum;
}

int FlashPlayer2::GetPercentLoaded()
{
    long aPercent = -1;
    if (m_pFlashInterface != NULL)
        m_pFlashInterface->PercentLoaded(&aPercent);
    return aPercent;
}

std::string FlashPlayer2::GetCurrentLabel(const std::string& theTimeline)
{
    BSTR aBStr = L"";
    if (m_pFlashInterface != NULL)
...
LONG FlashPlayer2::GetBackgroundColor()
{
...
void FlashPlayer2::SetBackgroundColor(LONG theColor)
{
    if (m_pFlashInterface != NULL)
        m_pFlashInterface->put_BackgroundColor(theColor);
}

BOOL FlashPlayer2::GetLoop()
{
    short aLoop = 0;
    if (m_pFlashInterface != NULL)
...
void FlashPlayer2::SetLoop(BOOL bLoop)
{
    if (m_pFlashInterface != NULL)
        m_pFlashInterface->put_Loop(bLoop==TRUE);
}

void FlashPlayer2::CallFrame(const std::string& theTimeline, int theFrameNum)
{
    if (m_pFlashInterface != NULL)
        m_pFlashInterface->TCallFrame(_bstr_t(theTimeline.c_str()), theFrameNum);
}

void FlashPlayer2::CallLabel(const std::string& theTimeline, const std::string& theLabel)
{
    if (m_pFlashInterface != NULL)
        m_pFlashInterface->TCallLabel(_bstr_t(theTimeline.c_str()), _bstr_t(theLabel.c_str()));
}

std::string FlashPlayer2::GetVariable(const std::string& theName)
{
    BSTR aBStr = L"";
    if (m_pFlashInterface != NULL)
...
void FlashPlayer2::SetVariable(const std::string& theName, const std::string& theValue)
{
    if (m_pFlashInterface != NULL)
        m_pFlashInterface->SetVariable(_bstr_t(theName.c_str()), _bstr_t(theValue.c_str()));
}

void FlashPlayer2::MouseMove(int x, int y)
{	
    LRESULT aResult;
    m_pWindowlessObject->OnWindowMessage(WM_MOUSEMOVE, 0, MAKELPARAM(x, y), &aResult);
}

void FlashPlayer2::MouseDown(int x, int y)
{
    LRESULT aResult;
    m_pWindowlessObject->OnWindowMessage(WM_LBUTTONDOWN, MK_LBUTTON, MAKELPARAM(x, y), &aResult);
}

void FlashPlayer2::MouseUp(int x, int y)
{
    LRESULT aResult;
    m_pWindowlessObject->OnWindowMessage(WM_LBUTTONUP, 0, MAKELPARAM(x, y), &aResult);
}

BOOL FlashPlayer2::Update()
{
    int nCurrentFrames = GetCurrentFrame();
    int nTotalFrames = GetTotalFrames();
    int nFrameNum = GetFrameNum();

    if (m_State == STATE_PLAYING)
    {
        VARIANT_BOOL isPlaying = 0;
        m_pFlashInterface->get_Playing(&isPlaying);	
        if (!isPlaying)
        {
...
BOOL FlashPlayer2::IsRender(RECT& rcDirty)
{
    rcDirty = m_rcFlashDirty;
    if (m_bFlashDirty)
    {
        m_bFlashDirty = FALSE;
        return TRUE;
...
BOOL FlashPlayer2::Render(HDC hDC,LONG x,LONG y,BOOL bClear)
{
    ...
    if (aViewObject != NULL)
    {
        ...
        if (bClear)
        {
            RECT aFill = {x, y, x+m_nWidth, y+m_nHeight};
            FillRect(hDC,&aFill,(HBRUSH)GetStockObject(WHITE_BRUSH));
        }

        RECTL aRect = {x, y, x+m_nWidth, y+m_nHeight};

        HRESULT aResult = aViewObject->Draw(DVASPECT_CONTENT, 1, NULL, NULL, hDC, hDC, &aRect, NULL, NULL, NULL);
...
void FlashPlayer2::SaveFlashBitmapBuffer(char* lpsFileName)
{
