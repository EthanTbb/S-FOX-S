...
BEGIN_MESSAGE_MAP(CDlgBank, CSkinDialog)
	...
	ON_BN_CLICKED(IDC_CHECK_ALL_GOLD, OnBnClickedCheck)
...
{
	if(IsButtonSelected(IDC_CHECK_ALL_GOLD))
	{
...
void CDlgBank::OnEnChangeInCount()
{
	...
	m_lGameGold=((m_pMeUserData->GetUserScore()<0)?0:m_pMeUserData->GetUserScore());
	m_lStorageGold=m_pMeUserData->GetUserInsure();
	LONGLONG lInCount=(m_bBankStorage)?m_lGameGold:m_lStorageGold;
...
void CDlgBank::ShowBank(BOOL bShow)
{
	GetDlgItem(IDC_USER_PASSWORD)->SetWindowText(TEXT(""));
	ShowWindow(bShow);
}
...
void CDlgBank::ShowItem()
{
	...
	if(m_bBankStorage)
	{
		...
		m_btOK.SetButtonImage(IDB_BT_SAVEGOLD, hInstance, false,false);
	...
	else
	{
		...
		m_btOK.SetButtonImage(IDB_BT_GETGOLD, hInstance, false,false);
		m_btCancel.SetButtonImage(IDB_BT_CLOSE,hInstance,false,false);
...
void CDlgBank::SetBankerActionType(bool bStorage) 
{
	...
	((CButton*)GetDlgItem(IDC_CHECK_ALL_GOLD))->SetCheck(0);
