    ...
    case GS_TK_FREE:    //空闲状态
    {
      ...
      if(IsLookonMode()==false)
      {
        if(m_pIClientKernel->GetServerAttribute()->wServerType != GAME_GENRE_MATCH &&
           CServerRule::IsAllowAvertCheatMode(m_pIClientKernel->GetServerAttribute()->dwServerRule) != true)
        {
...
void CGameClientEngine::UpdateScoreControl(LONGLONG lScore[],BOOL bShow)
{
  if(lScore!=NULL)
  {
    TCHAR tsz[MAX_JETTON_AREA][LEN_ACCOUNTS];
    ZeroMemory(tsz,sizeof(tsz));
    for(BYTE i=0; i<MAX_JETTON_AREA; i++)
    {
      _sntprintf(tsz[i],sizeof(tsz[i]),TEXT("%d"),lScore[i]);
    }
...
LRESULT CGameClientEngine::OnAddScore(WPARAM wParam, LPARAM lParam)
{
  ...
  {
    lCurrentScore=__max(m_lTurnMaxScore/3,1L);
  }
  else if(bTemp==3)
  {
