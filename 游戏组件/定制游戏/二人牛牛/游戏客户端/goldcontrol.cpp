...
{
  LONGLONG lGold=0L;
  for(LONGLONG i=0; i<CountArray(m_lGoldCell); i++)
  {
    lGold+=m_lGoldCell[i]*(LONGLONG)pow(10L,(int)i);
  }
  return lGold;
}
...
void CGoldControl::DrawGoldCell(CDC * pDC, int nXPos, int nYPox, LONGLONG lGold)
{
  ...
  {
    TCHAR szBuffer[12];
    _sntprintf(szBuffer,sizeof(szBuffer),TEXT("%d"),lGold);
...
void CGoldControl::OnPaint()
{
  ...
  //提示信息
  TCHAR szBuffer[50];
  _sntprintf(szBuffer,sizeof(szBuffer),TEXT("最多：%ld"),m_lMaxGold);
...
void CGoldControl::OnLButtonUp(UINT nFlags, CPoint point)
{
  ...
  if(iCellPos>3)
  {
    iCellPos=iCellPos-(iCellPos-1)/3;
  }
