...
void MakeStringToNum(CString str,SCORE &Num)
{
	...
	myscanf(str1.GetBuffer(),str1.GetLength(),_TEXT("%I64d"),&Num);
}
...
void CDlgBank::OnBnClickedOk()
{
	...
	if (false==m_bBankStorage)//取款
	{		
		//发送信息
		CMD_GR_C_TakeScoreRequest BankTakeGold;
		ZeroMemory(&BankTakeGold, sizeof(BankTakeGold));
		BankTakeGold.lTakeScore = m_lInCount;
		BankTakeGold.cbActivityGame=TRUE;
		CopyMemory(BankTakeGold.szInsurePass,szPassword,sizeof(BankTakeGold.szInsurePass));
		m_pIClientKernel->SendSocketData(MDM_GR_INSURE, SUB_GR_TAKE_SCORE_REQUEST, &BankTakeGold, sizeof(BankTakeGold));
	}
	else//存款
	{
		//状态判断
		...
		//发送信息
		CMD_GR_C_SaveScoreRequest BankStorage;
		ZeroMemory(&BankStorage, sizeof(BankStorage));
		BankStorage.cbActivityGame=TRUE;
		BankStorage.lSaveScore = m_lInCount;		
		m_pIClientKernel->SendSocketData(MDM_GR_INSURE, SUB_GR_SAVE_SCORE_REQUEST, &BankStorage, sizeof(BankStorage));
	}
...
//		m_bDrawAllGold=((CButton*)(GetDlgItem(IDC_CHECK_ALL)))->GetCheck()==1?true:false;
	//	((CButton*)(GetDlgItem(IDC_CHECK_ALL)))->SetCheck(m_bStorageAllGold?1:0);
		...
		GetDlgItem(IDC_CHECK_ALL)->SetWindowText(TEXT("存入所有金币"));
	...
	//	((CButton*)(GetDlgItem(IDC_CHECK_ALL)))->SetCheck(m_bDrawAllGold?1:0);
		...
		GetDlgItem(IDC_CHECK_ALL)->SetWindowText(TEXT("取出所有金币"));
