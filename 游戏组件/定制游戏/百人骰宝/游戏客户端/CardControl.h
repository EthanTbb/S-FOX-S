...
public:
	...
	CPoint							m_MovePoint;
	CPoint							m_DownPoint;
	tagMoveCard						m_MoveCard;

	//状态变量
protected:
	bool							m_bShowHand;						//显示手型
	...
	//移动扑克
protected:
	int								m_nMoveStep;						//移动步进
	int								m_nMaxOffset;						//最大偏移

	//扑克数据
public:
	WORD						m_wCardCount;
	CWHArray<CPoint>		m_CardOffSetPos;
	CWHArray<tagCardItem> m_CardItemArray;
	CWHArray<tagCardItem> m_CardTempItemArray;
	//选择变量
protected:
...
protected:
	...
	CBitImage	static				m_ImageCard;						//图片资源
	CBitImage						m_ImageOpenCardHandle;				//图片资源  IDB_HAND
...
public:
	...
	bool ReSetCardCtrl();
	//显示手型
	bool ShowHand(bool bShow);
	//设置扑克
	bool SetCardData(WORD wCardCount);
	//设置模式
	bool SetCardMode(bool bSmallMode);
	//设置扑克
	bool ReplaceCard(BYTE cbCardData,int nIndex);
	//设置扑克
	...
	//移动信息
	bool SetMoveInfo(int nStep,int MaxOffset);
...
public:
	//移动扑克
	bool MoveCard(BYTE cbCardIndex);
	//设置方向
...
public:
	...
	//拷贝扑克
	void OnCopyCard(int nCardNum=1);
};
