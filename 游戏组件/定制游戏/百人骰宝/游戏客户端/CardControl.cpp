...
//设置扑克
bool CCardControl::ReplaceCard(BYTE cbCardData,int nIndex)
{
	if(m_CardItemArray.GetCount()>nIndex)
	{
		m_CardItemArray[nIndex].cbCardData=cbCardData;
		m_CardTempItemArray[nIndex].cbCardData=cbCardData;
		return true;
	}
	return false;
}

bool CCardControl::ReSetCardCtrl()
{
	m_wCardCount=0;
	m_nMoveStep=0;
	m_nMaxOffset=0;
	m_CardOffSetPos.SetSize(0);
	m_CardTempItemArray.SetSize(0);
...
bool CCardControl::SetMoveInfo(int nMoveStep,int nMaxOffset)
{
	m_nMaxOffset=nMaxOffset;
	m_nMoveStep=nMoveStep;
	return true;
}

bool CCardControl::MoveCard(BYTE cbCardIndex)
{
    if(m_wCardCount>cbCardIndex)
	{
		
		m_CardOffSetPos[cbCardIndex].x+=m_nMoveStep;
		if(m_CardOffSetPos[cbCardIndex].x>=m_nMaxOffset)
		{
			m_nMoveStep*=(-1);
		}

		if(m_CardOffSetPos[cbCardIndex].x<=0)
			return false;
	}
	return true;
}
...
VOID CCardControl::DrawCardControl(CDC * pDC)
{
	...
	for (WORD i=0;i<m_wCardCount;i++)
	{
		...
		m_ImageCard.TransDrawImage(pDC,m_nXDistance*i+15+OriginPoint.x+m_CardOffSetPos[i].x,OriginPoint.y+m_CardOffSetPos[i].y,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos,RGB(255,0,255));
		
		if(m_bShowHand)
			m_ImageOpenCardHandle.TransDrawImage(pDC,OriginPoint.x,OriginPoint.y-5,m_ImageOpenCardHandle.GetWidth(),m_ImageOpenCardHandle.GetHeight(),0,0,RGB(255,0,255));

		//绘画选择
		if ((m_wStartIndex<=i)&&(i<=m_wEndIndex))
		{
			CSize SizeSelectImage(ImageCardSelect.GetWidth(),ImageCardSelect.GetHeight());
			ImageCardSelect.DrawImage(pDC,OriginPoint.x+m_CardOffSetPos[i].x,OriginPoint.y+m_CardOffSetPos[i].y,SizeSelectImage.cx,SizeSelectImage.cy,0,0);
		}
	}
...
bool CCardControl::ShowHand(bool bShow)
{
	m_bShowHand=bShow;
	return true;
}
...
//拷贝缓存中的牌实现动画
void CCardControl::OnCopyCard(int nCardNum)
{
	WORD wCount = 0;
	BYTE cbIndex=0;
	for(cbIndex=0;cbIndex<nCardNum;cbIndex++)
	{
		wCount=(WORD)m_CardItemArray.GetCount();
		if(m_CardTempItemArray.GetCount()>wCount)
		{
			...
			//变量定义
			CPoint ptCardPos;
			ptCardPos.SetPoint(0,0);
			m_CardOffSetPos.Add(ptCardPos);
		}
		else	break;
	}
}
