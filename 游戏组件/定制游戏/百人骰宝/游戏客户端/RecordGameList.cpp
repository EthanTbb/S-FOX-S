...
//插入列表
void CGameRecord::FillGameRecourd(GameRecord & GameRecord)
{
	//变量定义
	WORD wListIndex=0;
	WORD wColumnCount=1;
	...
	//本局成绩
	myprintf(szBuffer,CountArray(szBuffer),TEXT("%ld"),GameRecord.lGameScore);
	m_RecordGameList.SetItem(nItemIndex,wColumnCount++,LVIF_TEXT,szBuffer,0,0,0,0);

	//本局下注
	myprintf(szBuffer,CountArray(szBuffer),TEXT("%ld"),GameRecord.lMyAddGold);
	m_RecordGameList.SetItem(nItemIndex,wColumnCount++,LVIF_TEXT,szBuffer,0,0,0,0);

	//闲家下注
	myprintf(szBuffer,CountArray(szBuffer),TEXT("%ld"),GameRecord.lDrawPlayerScore);
	m_RecordGameList.SetItem(nItemIndex,wColumnCount++,LVIF_TEXT,szBuffer,0,0,0,0);

	//平家下注
	myprintf(szBuffer,CountArray(szBuffer),TEXT("%ld"),GameRecord.lDrawTieScore);
	m_RecordGameList.SetItem(nItemIndex,wColumnCount++,LVIF_TEXT,szBuffer,0,0,0,0);

	//庄家下注
	myprintf(szBuffer,CountArray(szBuffer),TEXT("%ld"),GameRecord.lDrawBankerScore);
	m_RecordGameList.SetItem(nItemIndex,wColumnCount++,LVIF_TEXT,szBuffer,0,0,0,0);

	//闲家牌点
	myprintf(szBuffer,CountArray(szBuffer),TEXT("%ld"),GameRecord.cbPlayerPoint);
	m_RecordGameList.SetItem(nItemIndex,wColumnCount++,LVIF_TEXT,szBuffer,0,0,0,0);

	//庄家牌点
	myprintf(szBuffer,CountArray(szBuffer),TEXT("%ld"),GameRecord.cbBankerPoint);
	m_RecordGameList.SetItem(nItemIndex,wColumnCount++,LVIF_TEXT,szBuffer,0,0,0,0);
