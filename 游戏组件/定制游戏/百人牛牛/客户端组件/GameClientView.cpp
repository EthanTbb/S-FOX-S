...
void CGameClientView::OnSendMessage() 
{
	...
	if(false == EfficacyUserChat(str, 0)) return;
...
VOID CGameClientView::DrawGameView(CDC * pDC, INT nWidth, INT nHeight)
{
	...
	int nFlagIndex=0;
	if (cbGameStatus==GAME_SCENE_FREE) nFlagIndex=1;
	else if (cbGameStatus==GAME_SCENE_PLACE_JETTON) nFlagIndex=2;
	else if (cbGameStatus==GAME_SCENE_GAME_END) nFlagIndex=0;
	...
	if(m_wMeChairID!=INVALID_CHAIR)
	{
		...
		if (wUserTimer!=0) DrawTime(pDC, wUserTimer, 270, 65 );
	}
	...
	{
		if (m_ImageDispatchCardTip.IsNull()==false) m_ImageDispatchCardTip.DestroyImage();
...
int CGameClientView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	for (int i = 0;i<5;i++)m_ImageCardType[i].LoadImage(hInstance,TEXT("CARDTYPE"));
...
void CGameClientView::DrawType(CDC* pDC,WORD wChairID)
{
	...
	if(m_pGameClientDlg->GetGameStatus() != GAME_SCENE_GAME_END) return;
	for (int i = 0;i<5;i++)	
	{
		if(m_CardControl[i].m_blShowResult==false) continue;
		int iIndex = 0;
...
void CGameClientView::DrawNumberString(CDC * pDC, LONGLONG lNumber, INT nXPos, INT nYPos, bool bMeScore)
{
	...
	if ( bMeScore ) SizeScoreNumber.SetSize(m_ImageMeScoreNumber.GetWidth()/11, m_ImageMeScoreNumber.GetHeight());
		...
		}
		{
			...
			}else
			{
				...
				/*	int iType = m_GameLogic.GetCardType((const BYTE*)bcCardDataTmp,5,bcTmp);
					...
					{
						if(CT_SPECIAL_NIUNIU<=m_lUserCardType[m_OpenCardIndex%5]) nCardType=10;
						else nCardType =m_lUserCardType[m_OpenCardIndex%5]-CT_SPECIAL_NIU1+1;
					}
...
BOOL CGameClientView::OnSetCursor(CWnd * pWnd, UINT nHitTest, UINT uMessage)
{
	if (m_lCurrentJetton!=0L)
	{
		...
		{
			if (lLeaveScore>=10000000L) SetCurrentJetton(10000000L);
			else if (lLeaveScore>=5000000L) SetCurrentJetton(5000000L);
...
void CGameClientView::OnScoreMoveR()
{
	if(!IsZoomedMode())
	{
		...
		if ( nHistoryCount == NORMAL_MAX_FALG_COUNT ) return;
		m_nScoreHead = ( m_nScoreHead + 1 ) % MAX_SCORE_HISTORY;
		if ( nHistoryCount-1 == NORMAL_MAX_FALG_COUNT ) m_btScoreMoveR.EnableWindow(FALSE);	
	}
	else
	{
		int nHistoryCount = ( m_nRecordLast - m_nScoreHead + MAX_SCORE_HISTORY ) % MAX_SCORE_HISTORY;
		if ( nHistoryCount == ZOOMED_MAX_FALG_COUNT ) return;
		m_nScoreHead = ( m_nScoreHead + 1 ) % MAX_SCORE_HISTORY;
		if ( nHistoryCount-1 == ZOOMED_MAX_FALG_COUNT ) m_btScoreMoveR.EnableWindow(FALSE);	
	}
...
LONGLONG CGameClientView::GetUserMaxJetton()
{
	...
	if (m_wBankerUser!=INVALID_CHAIR) lBankerScore=m_lBankerScore*m_nEndGameMul/100;
