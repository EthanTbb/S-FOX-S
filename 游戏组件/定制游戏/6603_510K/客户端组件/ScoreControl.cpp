...
VOID CScoreControl::OnCreateRes(CWnd* pWnd)
{
	//加载资源
	HINSTANCE hResInstance=AfxGetInstanceHandle();
	m_ImageWinLose.LoadFromResource(pWnd,hResInstance,IDB_WIN_LOSE);
	m_ImageGameScore.LoadFromResource(pWnd,hResInstance,IDB_GAME_SCORE);
		return;
}
...
VOID CScoreControl::DrawScoreView(CDC * pDC, INT nXPos, INT nYPos)
{
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		m_ImageWinLose.TransDrawImage(pDC,nXBenchmark+225,nYBenchmark+80+i*30,nImageWidht,nImageHeight,nImageExcursion,0,RGB(255,0,255));
	}
