...
//绘画设置
void CCardControl::SetDrawHeight(DWORD dwDrawHeight)
{
  m_dwDrawHeight=dwDrawHeight;

  return;
}

//设置牛牛
void CCardControl::SetOX(bool bOX)
{
  m_bOX=bOX;
...
//设置快捷键
void CCardControl::SetKey(bool bKey)
{
  m_bKey=bKey;
...
{
  if(m_BenchmarkPos.x==nXPos && m_BenchmarkPos.y==nYPos && m_XCollocateMode==XCollocateMode && m_YCollocateMode==YCollocateMode)
  {
    return ;
  }
...
//获取弹起扑克数目
DWORD CCardControl::GetShootCount()
{
  //变量定义
...
//获取扑克
DWORD CCardControl::GetShootCard(BYTE bCardData[],bool bShoot)
{
  //变量定义
  DWORD bShootCount=0L;
  DWORD dwCardCount=(DWORD)m_CardDataItem.GetCount();
...
//设置扑克
DWORD CCardControl::SetCardData(const BYTE bCardData[], DWORD dwCardCount, bool bShow)
{
  //设置扑克
  ...
  for(DWORD i=0; i<dwCardCount; i++)
  {
    ...
    m_CardDataItem[i].bShowCard=bShow;
...
DWORD CCardControl::GetCardData(BYTE cbCardData[],DWORD dwMaxCount)
{
  ...
  for(DWORD i=0; i<dwCardCount; i++)
  {
    cbCardData[i]=m_CardDataItem[i].bCardData;
  }
...
//设置显示
VOID CCardControl::SetShowCard(bool bShow)
{
  for(WORD i = 0; i < m_CardDataItem.GetCount(); i++)
  {
    m_CardDataItem[i].bShowCard = bShow;
...
void CCardControl::OnPaint()
{
  ...
  if(m_dwDrawHeight>0)
  {
    rcClient.bottom=48+DRAW_HEIGHT-30;
  }
  ...
  for(INT i=0; i<m_CardDataItem.GetCount(); i++)
  {
    ...
    //图片位置
    if((m_bDisplay==true)&&(pCardItem->bCardData!=0)&&(pCardItem->bShowCard==true))
    {
...
void CCardControl::OnLButtonUp(UINT nFlags, CPoint Point)
{
  ...
  //获取接口
  if(CGameFrameEngine::GetInstance()==NULL)
  {
    return;
  }
  ...
  if(pCardItem->bShowCard == true)
  {
    ...
    {
      tagCardItem * pTempCardItem = &m_CardDataItem[i];
      if(pTempCardItem->bShowCard == false)
      {
        return;
      }
    }
    ...
    //判断牛牛
    DWORD dwShootCount=GetShootCount();
    bool bHaveOx = false;
    if(dwShootCount==2)
    {
      //获取数据
      ...
      BYTE bCount=(BYTE)GetShootCard(bCardData,false);
      ASSERT(bCount==3);

      //十的倍数
      if(m_GameLogic.IsIntValue(bCardData,bCount))
      {
        bHaveOx = true;
        CGameFrameEngine::GetInstance()->PostMessage(IDM_OX_ENABLE,1,0);
      }
    }
    else if(dwShootCount==3)
    {
      //获取数据
      BYTE bCardData[MAX_COUNT];
      ZeroMemory(bCardData,sizeof(bCardData));
      BYTE bCount=(BYTE)GetShootCard(bCardData);
      ASSERT(bCount==3);
      ...
      {
        bHaveOx = true;
        CGameFrameEngine::GetInstance()->PostMessage(IDM_OX_ENABLE,1,1);
      }
    }

    if(!bHaveOx)
    {
      CGameFrameEngine::GetInstance()->PostMessage(IDM_OX_ENABLE,0,0);
    }
  }
  else
  {
    pCardItem->bShowCard=!pCardItem->bShowCard;
  }
