...
// 	//位置调整
	...
	m_btClose.SetButtonImage(IDB_BT_CLOSE_BUTTON,AfxGetInstanceHandle(),false,false);
	m_btMin.SetButtonImage(IDB_BT_MIN_BUTTON,AfxGetInstanceHandle(),false,false);
	m_btSound.SetButtonImage((pGlobalUnits->m_bMuteStatuts)?IDB_BT_SOUND_CLOSE:IDB_BT_SOUND_OPEN,AfxGetInstanceHandle(),false,false);
...
LRESULT	CGameFrameWindow::OnSetTextMesage(WPARAM wParam, LPARAM lParam)
{
	...
	((CGameClientView*)m_pIGameFrameView)->SendMessage(IDM_SET_CAPTION,wParam,lParam);
	//更新标题
	Invalidate(TRUE);
