	...
	case IDC_SOUND:
		{
			...
			IGameFrameView * pIGameFrameView=(IGameFrameView *)pGlobalUnits->QueryGlobalModule(MODULE_GAME_FRAME_VIEW,IID_IGameFrameView,VER_IGameFrameView);
			CGameClientView *pGameClientView = (CGameClientView*)pIGameFrameView;
			pGameClientView->SendEngineMessage(IDM_GAME_SOUND,0,0);

			m_bPlaySound = !m_bPlaySound;
			m_btSound.SetButtonImage((!pGlobalUnits->m_bMuteStatuts)?IDB_PLAY_SOUND:IDB_STOP_SOUND,AfxGetInstanceHandle(),false,false);
...
VOID CGameFrameWindow::RectifyControl(INT nWidth, INT nHeight)
{
	...
	DeferWindowPos(hDwp, m_btMin, NULL,nWidth-110,0,0,0, uFlags);
	DeferWindowPos(hDwp, m_btSound, NULL, nWidth-75,0,0,0, uFlags);
	DeferWindowPos(hDwp, m_btClose, NULL, nWidth-40,0,0,0, uFlags);
	DeferWindowPos(hDwp, m_btOption, NULL, nWidth-145,0,0,0, uFlags);
...
INT CGameFrameWindow::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	::ShowWindow(m_pIGameFrameView->GetGameViewHwnd(),SW_HIDE);
	//用户接口
	...
	//
	//m_PngFrameTop.LoadImage(this,TEXT("FRAME_TOP"));
	...
	m_btMin.Create(TEXT(""),WS_CHILD|WS_VISIBLE,rcCreate,this,IDC_BT_MIN);
	m_btClose.Create(TEXT(""),WS_CHILD|WS_VISIBLE,rcCreate,this,IDC_BT_CLOSE);
	m_btOption.Create(TEXT(""),WS_CHILD|WS_VISIBLE,rcCreate,this,IDC_OPTION);

	m_btSound.SetButtonImage((!pGlobalUnits->m_bMuteStatuts)?IDB_PLAY_SOUND:IDB_STOP_SOUND,AfxGetInstanceHandle(),false,false);
	m_btMin.SetButtonImage(IDB_BT_MIN,AfxGetInstanceHandle(),false,false);
	m_btClose.SetButtonImage(IDB_BT_CLOSE,AfxGetInstanceHandle(),false,false);
...
{
//	AfxMessageBox(TEXT("DFD"),0,0);
//	if(point.y<500) return HTCAPTION;
