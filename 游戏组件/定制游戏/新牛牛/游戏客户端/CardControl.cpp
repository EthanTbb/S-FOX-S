...
CCardControl::CCardControl()
{
	...
	if (m_bLoad==false)
	{
		//加载资源
		m_bLoad=true;
		if (GetSystemMetrics(SM_CXSCREEN)>=1024) m_ImageCard.LoadImage(AfxGetInstanceHandle(), _T("GAME_CARD"));
		else m_ImageCard.LoadImage(AfxGetInstanceHandle(),_T("GAME_CARD"));
...
void CCardControl::OnPaint()
{
	...
	if(m_dwDrawHeight>0)rcClient.bottom=48+DRAW_HEIGHT-30;
...
void CCardControl::OnLButtonUp(UINT nFlags, CPoint Point)
{
	...
	//获取接口
	if(CGameFrameEngine::GetInstance()==NULL)return;
	...
	if(!bHaveOx)CGameFrameEngine::GetInstance()->PostMessage(IDM_OX_ENABLE,0,0);
