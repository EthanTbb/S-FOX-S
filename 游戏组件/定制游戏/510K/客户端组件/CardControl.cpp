...
void CCardControl::OnCreateRes(CWnd *pWnd){
	//加载资源
	HINSTANCE hResInstance=AfxGetInstanceHandle();
	m_ImageCard.LoadFromResource(pWnd,hResInstance,(m_bSmallMode==false)?IDB_GAME_CARD:IDB_GAME_CARD_SMALL);
...
//设置模式
bool CCardControl::SetCardMode(CWnd *pWnd,bool bSmallMode)
{
	//设置模式
...
VOID CCardControl::DrawCardControl(CDC * pDC)
{
	...
	for (WORD i=0;i<m_wCardCount;i++)
	{
		...
		//绘画扑克
		m_ImageCard.TransDrawImage(pDC,OriginPoint.x+nXDrawPos,OriginPoint.y+nYDrawPos,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos,RGB(255,0,255));
