  ...
  if(!m_bStopDraw)
  {
    ...
    for(WORD i=0; i<GAME_PLAYER; i++)
    {
      ...
      if(pClientUserItem!=NULL)
      {
        ...
        if(m_btStart.IsWindowVisible()==TRUE || m_cbPlayStatus[i]==FALSE)
        {
          lTempScore=0;
        }
        LONGLONG lLeaveScore=pClientUserItem->GetUserScore()-lTempScore;
        if(m_lStopUpdataScore[i]>0)
        {
          lLeaveScore=m_lStopUpdataScore[i]-m_lTableScore[i];
        }
...
{
  //动画判断
  for(int i=0; i<GAME_PLAYER; i++) if(m_wFlashUser[i]<GAME_PLAYER)
    {
      break;
    }
  if(i==GAME_PLAYER)
...
void CGameClientView::DrawUser(CDC * pDC,INT nWidth, INT nHeight)
{
  ...
  for(int i = 0; i < GAME_PLAYER; ++i)
  {
    ...
    if(m_ImageUser[i].IsNull() || m_ImageEye[i].IsNull())
    {
      continue;
    }
...
void CGameClientView::LoadUserImage(WORD wChairID,INT nGender)
{
  ...
  //卸载资源
  if(m_ImageUser[wChairID].IsNull()!=true)
  {
    m_ImageUser[wChairID].DestroyImage();
  }
  if(m_ImageEye[wChairID].IsNull()!=true)
  {
    m_ImageEye[wChairID].DestroyImage();
  }
