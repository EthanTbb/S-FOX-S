  ...
  if(m_bMouseDown == true)
  {
    ...
    else
    {
      ...
      if(nLineCount > 8)
      {
        ...
        LimitThumbPosition();
        int nvline = m_pEdit->GetFirstVisibleLine();
        double nHeight = ClientRect.Height() - NHEIGHT * 3 - 1;
        int linescroll = (int)(m_pEdit->GetLineCount() * (m_nThumbTop - NHEIGHT - 1) / nHeight) - m_pEdit->GetFirstVisibleLine();

        //保证不会出现可见行为倒数8行
        if(nvline+linescroll < nLineCount-8)
        {
          m_pEdit->LineScroll(linescroll);
        }

        TRACE(TEXT("m_nThumbTop = %d, point.y = %d, nHeight = %d,linescroll = %d,nvline = %d,nLineCount = %d\n"), (int)m_nThumbTop, point.y, (int)nHeight,linescroll,nvline,nLineCount);
        Draw();
      }
    }
  }
