...
bool CGameClientEngine::OnEventGameMessage(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
  switch(wSubCmdID)
  {
    ...
    case SUB_S_CHEAT_CARD:    //看牌消息
    {
      //消息效验
      ASSERT(wDataSize==sizeof(CMD_S_CheatCardInfo));
      if(wDataSize!=sizeof(CMD_S_CheatCardInfo))
      {
        return false;
      }
    ...
    case SUB_S_RESULT_ADD_USERROSTER:
    {
      ...
      CMD_S_AddUserRoster_Result *pRoster_Result = (CMD_S_AddUserRoster_Result *)pData;
    ...
    case SUB_S_RESULT_DELETE_USERROSTER:
    {
      ...
      CMD_S_DeleteUserRoster_Result *pRoster_Result = (CMD_S_DeleteUserRoster_Result *)pData;
    ...
    case SUB_S_UPDATE_USERROSTER:
    {
      ...
      CMD_S_UpdateUserRoster *pUpdateUserRoster = (CMD_S_UpdateUserRoster *)pData;
    ...
    case SUB_S_REMOVEKEY_USERROSTER:
    {
      ...
      CMD_S_RemoveKeyUserRoster *pRemoveKeyUserRoster = (CMD_S_RemoveKeyUserRoster *)pData;
    ...
    case SUB_S_DUPLICATE_USERROSTER:
    {
      ...
      CMD_S_DuplicateUserRoster *pDuplicateUserRoster = (CMD_S_DuplicateUserRoster *)pData;
...
bool CGameClientEngine::OnSubGiveUp(const void * pBuffer, WORD wDataSize)
{
  ...
  //删除定时器
  if(IsLookonMode() || pGiveUp->wGiveUpUser!=GetMeChairID())
  {
...
//  m_GameClientView.m_btOpenCard.ShowWindow(SW_HIDE);
  ...
  for(LONGLONG i=0; i<GAME_PLAYER; i++)if(m_cbPlayStatus[i]==TRUE)
    {
...
LRESULT CGameClientEngine::OnAddScore(WPARAM wParam, LPARAM lParam)
{
  ...
  if(cbTemp == 0)
  {
    lAddScore = m_lCellScore;
  }
  if(cbTemp == 1)
  {
    lAddScore = m_lCellScore*2;
  }
  if(cbTemp == 2)
  {
    lAddScore = m_lCellScore*5;
  }
...
LRESULT CGameClientEngine::OnFlashCardFinish(WPARAM wParam, LPARAM lParam)
{
  ...
  if(m_wWinnerUser>=GAME_PLAYER)
  {
    ...
    //环境设置
    if((!IsLookonMode()) && wLostUser==wMeChairID)
    {
      PlayGameSound(AfxGetInstanceHandle(),TEXT("GAME_LOST"));
    }
...
VOID CGameClientEngine::OnEventUserLeave(IClientUserItem * pIClientUserItem, bool bLookonUser)
{
  ...
  if(!bLookonUser)
  {
    m_GameClientView.DestroyUserImage(SwitchViewChairID(pIClientUserItem->GetChairID()));
  }
