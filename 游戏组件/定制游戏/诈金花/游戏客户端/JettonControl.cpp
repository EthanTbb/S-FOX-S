...
{
  //效验是否有待决动画
  if(m_arExcusionsPend.GetCount() == 0)
  {
    return;
  }
  ...
  for(; ;)
  {
    ...
    while(lJetScore != 0)
    {
      ...
      if(lJetScore > 0L)
      {
        ...
        {
          nStepCount = abs(ptTo.y-ptFrom.y)/MOVE_STEP_LEN;
        }
...
BOOL CJettonControl::DistillJetIndex(LONGLONG lScore,INT_PTR &nJetIndex)
{
  ...
  do
  {
    ...
    if(lScore >= m_lJetonIndex[wJettonIndex])
    {
      break;
    }
...
{
  //绘画判断
  if(m_lScore<=0L&&m_arIndexPending.GetCount()==0)
  {
    return;
  }
