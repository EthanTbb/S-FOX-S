...
void CGoldControl::SetMaxGold(LONGLONG lMaxGold)
{
  ...
  //设置变量
  m_lMaxGold=lMaxGold;
  if(m_lMaxGold>999999999L)
  {
    m_lMaxGold=999999999L;
  }
  memset(m_lGoldCell,0,sizeof(m_lGoldCell));
...
void CGoldControl::OnPaint()
{
  ...
  {
    int iX=0;
    if(m_lGoldCount[0] == 100)
    {
      iX=3;
    }
    else if(m_lGoldCount[0] == 1000)
    {
      iX=6;
    }
    else if(m_lGoldCount[0] == 10000)
    {
      iX=9;
    }
    else if(m_lGoldCount[0] == 100000)
    {
      iX=12;
    }
    else if(m_lGoldCount[0] == 1000000)
    {
      iX=15;
    }
...
void CGoldControl::OnLButtonUp(UINT nFlags, CPoint point)
{
  ...
  if((point.x <=15) || (point.x>=nX))
  {
    return;
  }
  if((point.y>=77) || (point.y<=nY))
  {
    return;
  }
