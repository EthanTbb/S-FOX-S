...
BOOL CListCtrlCl::OnEraseBkgnd(CDC* pDC)
{
	...
	//pDC->StretchBlt(0,0,clientRect.Width(),clientRect.Height(),&bitmapDC,0,0,clientRect.Width(),clientRect.Height(),SRCCOPY);
...
{  
//	OldWinProc = (WNDPROC)SetWindowLong(m_hWnd,   GWL_WNDPROC,   (LONG)NewWinProc);//WM_MOUSEACTIVATE   
...
void CListCtrlCl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	for ( int nCol=0; GetColumn(nCol, &lvc); nCol++)
	{
		...
		DrawText(lpDrawItemStruct->hDC, lpBuffer, int(mystrlen(lpBuffer)), 
			&rcItem, uFormat) ;
