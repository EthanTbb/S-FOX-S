...
CCardControl::CCardControl()
{
	...
	if (m_ImageCard.IsNull()) m_ImageCard.LoadImage(hResInstance,TEXT("GAME_CARD"));
...
CCardControl::~CCardControl()
{
	if (m_ImageCard.IsNull()==false) m_ImageCard.DestroyImage();
}
...
VOID CCardControl::DrawCardControl(CDC * pDC)
{
	...
	for (WORD i=0;i<m_wCardCount;i++)
	{
		...
		if(m_blGameEnd)
		{		m_ImageCard.DrawImage(pDC,OriginPoint.x+nXDrawPos+15,OriginPoint.y+nYDrawPos,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos);
		...
		}else
		{
			   if(i<3)
			   {
				   m_ImageCard.DrawImage(pDC,OriginPoint.x+nXDrawPos+25,OriginPoint.y+nYDrawPos,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos);
				...
				if(4==i)
				{
					m_ImageCard.DrawImage(pDC,OriginPoint.x+nXDrawPos+m_MovePoint.x+25,OriginPoint.y+nYDrawPos+m_MovePoint.y,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos);
					...
					m_ImageOpenCardHandle.DrawImage(pDC,OriginPoint.x-7,OriginPoint.y+nYDrawPos-8,m_ImageOpenCardHandle.GetWidth(),m_ImageOpenCardHandle.GetHeight(),0,0);
				}
				else
				{
					m_ImageCard.DrawImage(pDC,OriginPoint.x+nXDrawPos+25,OriginPoint.y+nYDrawPos,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos);
