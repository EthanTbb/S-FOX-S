...
CCardControl::CCardControl()
{
  ...
  m_ImageOpenCardHandle.LoadImage(hResInstance,TEXT("HAND"));
...
VOID CCardControl::DrawCardControl(CDC * pDC)
{
  ...
  {
    ImageCardSelect.LoadImage(AfxGetInstanceHandle(),_TEXT("CARD_SELECT"));
  }
  m_wCardCount=(WORD)m_CardItemArray.GetCount();
  //绘画扑克
  for(WORD i=0; i<m_wCardCount; i++)
  {
    ...
    if(m_blGameEnd)
    {
      m_ImageCard.DrawImage(pDC,OriginPoint.x+nXDrawPos+15,OriginPoint.y+nYDrawPos+12,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos);
    }
    else
    {
      //绘画扑克
      if(1==i)
      {
        m_ImageCard.DrawImage(pDC,OriginPoint.x+nXDrawPos+m_MovePoint.x+25,OriginPoint.y+nYDrawPos+12+m_MovePoint.y,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos);
        m_ImageOpenCardHandle.DrawImage(pDC,OriginPoint.x+1,OriginPoint.y+nYDrawPos,m_ImageOpenCardHandle.GetWidth(),m_ImageOpenCardHandle.GetHeight(),0,0);
      }
      else
      {
        m_ImageCard.DrawImage(pDC,OriginPoint.x+nXDrawPos+25,OriginPoint.y+nYDrawPos+12,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos);
      }
    }
    //绘画选择
  ...
  if((wShootCount>0)&&(wShootCount<m_wCardCount))
  {
    ...
    for(WORD i=0; i<m_wCardCount; i++)
    {
      if(CardItemTemp[i].bShoot==false)
      {
        ...
        //调整索引
        if(wNormalIndex==wTargerItem)
        {
          wNormalIndex=wTargerItem+wShootCount;
        }
      }
      else
      {
...
bool CCardControl::SwitchCardItem(WORD wSourceItem, WORD wTargerItem)
{
  ...
  if((wSourceItem>=m_wCardCount)||(wTargerItem>=m_wCardCount))
  {
    return false;
  }
