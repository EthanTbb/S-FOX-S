...
bool CTempldateHelper<IModeluInterface>::CreateInstance()
{
	...
	try
	{
		...
		if (CreateProc==NULL) 
		{
			_sntprintf(m_szDescribe,CountArray(m_szDescribe),TEXT("找不到组件创建函数“%hs”"),m_szCreateProc);
			return false;
		}
		...
		{
			_sntprintf(m_szDescribe,CountArray(m_szDescribe),TEXT("调用函数“%hs”生成对象失败"),m_szCreateProc);
			return false;
		}
	}
	...
	{ 
		_sntprintf(m_szDescribe,CountArray(m_szDescribe),TEXT("组件创建函数“%hs”产生未知异常错误，组件创建失败"),m_szCreateProc);
		return false;
	}
