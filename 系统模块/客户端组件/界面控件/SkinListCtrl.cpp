...
{
	//标题属性
	m_crTitle=RGB(254,216,65);
	m_crListTX=RGB(0,0,0);
	m_crListBK=RGB(189,173,139);
...
VOID CSkinHeaderCtrl::PreSubclassWindow()
{
	...
	if (pSkinAttribute!=NULL && pSkinAttribute->m_SkinImageM.IsNull()==false)
	{
		m_uRowHeight=pSkinAttribute->m_SkinImageM.GetHeight();
	}
	else
	{
		m_uRowHeight=0;
...
LRESULT CSkinHeaderCtrl::OnLayout(WPARAM wParam, LPARAM lParam)
{
	...
	int nHeight = m_uRowHeight>0?m_uRowHeight:pwpos->cy;
	pwpos->cy = nHeight; 
	prc->top = nHeight; 
	return lResult; 
}
...
VOID CSkinListCtrl::DrawCustomItem(CDC * pDC, LPDRAWITEMSTRUCT lpDrawItemStruct, CRect & rcSubItem, INT nColumnIndex)
{
	...
	//绘画文字
	rcSubItem.left+=5;
	pDC->DrawText(szString,lstrlen(szString),&rcSubItem,DT_CENTER| DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
