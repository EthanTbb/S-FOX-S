		...
		for ( int i = 0 ; i < lstrlen(strInfo); ++i )
		{
			...
			{				
				...
				if(rcDraw.top+LogFont.lfHeight*2+5>rcDraw.bottom)
				{
					...
					{
						int nPointNumber = 0;
						if( GetCharLength(strInfo[i], m_nWidth) == m_nWidth )
						{
							if( nLengthX - (m_nWidth/2) <= (lpRect->right - lpRect->left) )
							{
								strInfo.SetAt( i, '.' );
								nPointNumber += 1;
							}
						...
						for( int nIndexI = i - 1; nIndexI >= 0 && nPointNumber < 3; nIndexI-- )
						{
							if( GetCharLength(strInfo[nIndexI], m_nWidth) == m_nWidth )
							{
								if( nPointNumber <= 1 )
								{
									strInfo.SetAt( nIndexI, '.' );
									strInfo.Insert( nIndexI, TEXT(".") );
									nPointNumber += 2;
									nDrawCount += 1;
								}
								else
					...
					CD2DEngine::GetD2DEngine()->DrawText(m_pWnd, m_pD2DFont, strInfo.Left(nDrawCount), rcDraw, colorref, nFormat);
					bComplete=true;
					break;
				}
				else
		...
		for ( int i = 0 ; i < lstrlen(strInfo); ++i )
		{
			...
			{				
				...
				if(rcDraw.top+LogFont.lfHeight*2+5>rcDraw.bottom)
				{
					...
					{
						int nPointNumber = 0;
						if( GetCharLength(strInfo[i], nWidth) == nWidth )
						{
							if( nLengthX - (nWidth/2) <= (lpRect->right - lpRect->left) )
							{
								strInfo.SetAt( i, '.' );
								nPointNumber += 1;
							}
						...
						for( int nIndexI = i - 1; nIndexI >= 0 && nPointNumber < 3; nIndexI-- )
						{
							if( GetCharLength(strInfo[nIndexI], nWidth) == nWidth )
							{
								if( nPointNumber <= 1 )
								{
									strInfo.SetAt( nIndexI, '.' );
					...
					CD2DEngine::GetD2DEngine()->DrawText(pWnd, pD2DFont, strInfo.Left(nDrawCount), rcDraw, colorref, nFormat);
					bComplete=true;
					break;
				}
				else
