...
WORD CPlazaUserManager::m_wObjectRefCount=0;							//存储数组
...
CPlazaUserManager::~CPlazaUserManager()
{
	...
	//引用递减
	if(--m_wObjectRefCount==0)
	{
		//释放资源
		for(INT_PTR nIndex=0;nIndex<m_UserItemStorage.GetCount();nIndex++)
		{
			SafeDelete(m_UserItemStorage[nIndex]);
		}
...
//增加用户
IClientUserItem * CPlazaUserManager::ActiveUserItem(tagUserInfo & UserInfo,tagCustomFaceInfo & CustomFaceInfo,bool bIsAndroid)
{
	//变量定义
	...
	//扩展属性
	pClientUserItem->m_bIsAndroid = bIsAndroid;
...
//更新积分
bool CPlazaUserManager::UpdateUserItemScore(IClientUserItem * pIClientUserItem, tagUserScore * pUserScore)
{
	//效验参数
	...
	pUserInfo->dBeans=pUserScore->dBeans;
	...
	pUserInfo->lIntegralCount=pUserScore->lIntegralCount;
...
//更新状态
bool CPlazaUserManager::UpdateUserItemStatus(IClientUserItem * pIClientUserItem, tagUserStatus * pUserStatus)
{
	//效验参数
...
//更新属性
bool CPlazaUserManager::UpdateUserItemAttrib(IClientUserItem * pIClientUserItem, tagUserAttrib * pUserAttrib)
{
	//效验参数
	ASSERT(pUserAttrib!=NULL);
	ASSERT(pIClientUserItem!=NULL);
...
CGameUserManager::~CGameUserManager()
{
	...
	for(INT_PTR nIndex=0;nIndex<m_UserItemStorage.GetCount();nIndex++)
	{
		CClientUserItem *pClientUserItem=m_UserItemStorage[nIndex];
		if (pClientUserItem!=NULL) SafeDelete(pClientUserItem);
	}
	m_UserItemStorage.RemoveAll();
...
//增加用户
IClientUserItem * CGameUserManager::ActiveUserItem(tagUserInfo & UserInfo, tagCustomFaceInfo & CustomFaceInfo, BYTE cbCompanion, LPCTSTR pszUserNote,bool bIsAndroid)
{
	//效验状态
...
//更新积分
bool CGameUserManager::UpdateUserItemScore(IClientUserItem * pIClientUserItem, tagUserScore * pUserScore)
{
	//效验参数
	ASSERT(pUserScore!=NULL);
	ASSERT(pIClientUserItem!=NULL);
...
//更新状态
bool CGameUserManager::UpdateUserItemStatus(IClientUserItem * pIClientUserItem, tagUserStatus * pUserStatus)
{
	//效验参数
	ASSERT(pUserStatus!=NULL);
	ASSERT(pIClientUserItem!=NULL);
	...
	//通知更新
	ASSERT(m_pIUserManagerSink!=NULL);
	if (m_pIUserManagerSink!=NULL) m_pIUserManagerSink->OnUserItemUpdate(pIClientUserItem,UserStatus);

	return true;
}

//更新属性
bool CGameUserManager::UpdateUserItemAttrib(IClientUserItem * pIClientUserItem, tagUserAttrib * pUserAttrib)
{
	//以往数据
	tagUserAttrib UserAttrib;
	UserAttrib.cbCompanion=pIClientUserItem->GetUserCompanion();
	...
	//通知更新
	ASSERT(m_pIUserManagerSink!=NULL);
	if (m_pIUserManagerSink!=NULL) m_pIUserManagerSink->OnUserItemUpdate(pIClientUserItem,UserAttrib);
...
bool CGameUserManager::UpdateUserCustomFace(IClientUserItem * pIClientUserItem, DWORD dwCustomID, tagCustomFaceInfo & CustomFaceInfo)
{
	...
	//通知更新
	ASSERT(m_pIUserManagerSink!=NULL);
	if (m_pIUserManagerSink!=NULL) m_pIUserManagerSink->OnUserFaceUpdate(pIClientUserItem);
