...
//广告偏移
#define	BILL_T						10									//广告上高
#define	BILL_L						10									//广告左宽
#define	BILL_R						10									//广告右宽
#define	BILL_B						0									//广告下高
#define	BILL_H						82									//广告下高
//控件标示
...
BOOL CGameFrameUserView::OnEraseBkgnd(CDC * pDC)
{
	...
	if (m_pICurrentUserItem!=NULL)
	{
		...
		_sntprintf(szString,CountArray(szString),TEXT("级别：%s [ %s ]"),pszGameLevel,pGameAttribute->szGameName);
		...
		//成绩
		_sntprintf(szString,CountArray(szString),TEXT("成绩：%I64d"),pUserInfo->lScore);
		...
		_sntprintf(szString,CountArray(szString),TEXT("胜率：%.2f%%"),fUserWinRate);
		...
		_sntprintf(szString,CountArray(szString),TEXT("逃跑率：%.2f%%"),fUserFleeRate);
