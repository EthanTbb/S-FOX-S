	...
	case SUB_GR_USER_INSURE_INFO:
		{
			...
			m_DlgInsureGame.m_lUserScore=pUserInsureInfo->lUserScore;
			m_DlgInsureGame.m_lUserInsure=pUserInsureInfo->lUserInsure;

			//更新界面
			m_DlgInsureGame.UpdateUI();
		}
		break;
	case SUB_GR_USER_INSURE_SUCCESS:
		{
			...
			//设置变量
			m_DlgInsureGame.m_lUserScore=pUserInsureSuccess->lUserScore;
			m_DlgInsureGame.m_lUserInsure=pUserInsureSuccess->lUserInsure;
...
void CGameFrameView::SetInsureOption(bool blCanGetOnlyFree, bool blCanStore)
{
	...
	//更新显示
	if(m_DlgInsureGame.m_hWnd !=NULL) m_DlgInsureGame.ShowItem();
}
...
void CGameFrameView::ShowInsureSave(IClientKernel *	pIClientKernel, CPoint pos)
{
	...
	if (!m_DlgInsureGame.IsWindowVisible())
	{
