...
//绘制数字
VOID DrawNumber(CDC * pDC,INT nXPos,INT nYPos,CPngImage * pNumberImage,LPCTSTR pszNumber,INT nNumber,LPCTSTR pszFormat,UINT nFormat)
{
	//参数校验
...
VOID CGameFrameViewGDI::DrawUserClock(CDC * pDC, INT nXPos, INT nYPos, WORD wUserClock)
{
	...
	if ((wUserClock>0)&&(wUserClock<100))
	{
		...
		if (RealizeWIN7() && CD2DEngine::GetD2DEngine())
		{
			...
			m_ImageClockItemEx.DrawImage(pDC,nXDrawPos+16,nYDrawPos+27,SizeClockItem.cx,SizeClockItem.cy,nClockItem1*SizeClockItem.cx,0);
			m_ImageClockItemEx.DrawImage(pDC,nXDrawPos+30,nYDrawPos+27,SizeClockItem.cx,SizeClockItem.cy,nClockItem2*SizeClockItem.cx,0);
		}
		else
		{
			//加载资源
...
VOID CGameFrameViewGDI::DrawUserAvatar(CDC * pDC, INT nXPos, INT nYPos, INT nWidth, INT nHeight, IClientUserItem * pIClientUserItem)
{
	...
	if (  RealizeWIN7() && CD2DEngine::GetD2DEngine() )
	{
		...
		{
			WORD wFaceID=pIClientUserItem->GetFaceID();
			m_ImgGroupUserFace[wFaceID].DrawImage(pDC,nXPos,nYPos,nWidth,nHeight,0,0,FACE_CX,FACE_CY);
			////变量定义
...
VOID CGameFrameViewGDI::DrawMatchInfo(CDC * pDC)
{
	...
	{
		CPngImage ImageWaitDistribute;
		ImageWaitDistribute.LoadImage( GetModuleHandle(GAME_FRAME_DLL_NAME),TEXT("WAIT_DISTRIBUTE"));
		ImageWaitDistribute.DrawImage(pDC,(rcClient.Width()-ImageWaitDistribute.GetWidth())/2,(rcClient.Height()-ImageWaitDistribute.GetHeight())/2);
	}
	...
	if(m_pMatchInfo!=NULL)
	{
		...
		CDFontEx DrawFont;
		DrawFont.CreateFont(this,TEXT("宋体"),12,12);
		pDC->SetTextColor(RGB(255,255,255));
		CString strNum;
		strNum.Format(TEXT("%d"),m_pMatchInfo->wGameCount);
		TextOut(pDC,10,32,strNum,&DrawFont); 
		pDC->SetTextColor(RGB(196,221,239));
		TextOut(pDC,38,10,m_pMatchInfo->szTitle[0],&DrawFont);
		TextOut(pDC,38,30,m_pMatchInfo->szTitle[2],&DrawFont);
		pDC->SetTextColor(RGB(255,221,35));
		TextOut(pDC,38,50,m_pMatchInfo->szTitle[3],&DrawFont);
		DrawFont.DeleteFont();
	}
	...
	if(m_pMatchWaitTip!=NULL)
	{
		...
		//创建字体
		CDFontEx DrawFont;
		DrawFont.CreateFont(this,TEXT("宋体"),20,20);
		pDC->SetTextColor(RGB(255,255,0));

		//绘画信息
		ImageWaitTip.AlphaDrawImage(pDC,nXPos,nYPos-20,220);
		DrawText(pDC,m_pMatchWaitTip->szMatchName,&CRect(nXPos+20,nYPos+10, nXPos+ImageWaitTip.GetWidth()-40,nYPos+40),DT_VCENTER|DT_SINGLELINE|DT_CENTER,&DrawFont);
		DrawFont.DeleteFont();
		...
		DrawText(pDC,TEXT("您目前积分："), &CRect(nXPos+18,nYPos+64, nXPos+ImageWaitTip.GetWidth()-36, nYPos+90),DT_TOP|DT_SINGLELINE|DT_END_ELLIPSIS,&DrawFont);
		DrawNumber(pDC,nXPos+90, nYPos+68, &ImageNumberOrange, TEXT("0123456789-"), (LONG)m_pMatchWaitTip->lScore, TEXT("%+d"), DT_LEFT|DT_VCENTER);

		DrawText(pDC,TEXT("排名："), &CRect(nXPos+228,nYPos+64, nXPos+ImageWaitTip.GetWidth()-36, nYPos+90),DT_TOP|DT_SINGLELINE|DT_END_ELLIPSIS,&DrawFont);
		DrawNumber(pDC,nXPos+288, nYPos+68, &ImageNumberOrange, TEXT("0123456789-"), m_pMatchWaitTip->wRank, TEXT("%+d"), DT_CENTER|DT_VCENTER);
		ImageLine.DrawImage(pDC, nXPos+293, nYPos+57);
		DrawNumber(pDC,nXPos+328, nYPos+68, &ImageNumberOrange, TEXT("0123456789-"), m_pMatchWaitTip->wUserCount, TEXT("%+d"), DT_CENTER|DT_VCENTER);		
		
		strDrawText.Format(TEXT("请耐心等待，还有      桌未完成比赛"));
		DrawText(pDC,strDrawText, &CRect(nXPos+18,nYPos+104, nXPos+ImageWaitTip.GetWidth()-36, nYPos+130),DT_TOP|DT_SINGLELINE|DT_END_ELLIPSIS,&DrawFont);
		DrawNumberString(pDC, ImageNumberGreen, m_pMatchWaitTip->wPlayingTable, nXPos+130,nYPos+108);
		strDrawText.Format(TEXT("您已经率先完成第             局，处于本桌第      名。"));
		DrawText(pDC,strDrawText, &CRect(nXPos+18,nYPos+144, nXPos+ImageWaitTip.GetWidth()-36, nYPos+200),DT_TOP|DT_SINGLELINE|DT_END_ELLIPSIS,&DrawFont);

		DrawNumber(pDC,nXPos+138, nYPos+148, &ImageNumberOrange, TEXT("0123456789-"), m_pMatchWaitTip->wCurGameCount, TEXT("%+d"), DT_CENTER|DT_VCENTER);
		ImageLine.DrawImage(pDC, nXPos+145, nYPos+140);

		DrawNumber(pDC,nXPos+158,nYPos+148,&ImageNumberOrange,TEXT("0123456789-"),m_pMatchWaitTip->wGameCount,TEXT("%+d"),DT_LEFT|DT_VCENTER);
		DrawNumberString(pDC, ImageNumberGreen, m_pMatchWaitTip->wCurTableRank, nXPos+290,nYPos+148);
		DrawText(pDC,TEXT("请等待其他桌游戏结束后，确定最后排名。"), &CRect(nXPos+18,nYPos+188, nXPos+ImageWaitTip.GetWidth()-36, nYPos+220),DT_TOP|DT_WORDBREAK,&DrawFont);
