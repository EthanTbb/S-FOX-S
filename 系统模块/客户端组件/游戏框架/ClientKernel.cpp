...
VOID * CClientKernel::QueryInterface(REFGUID Guid, DWORD dwQueryVer)
{
	...
	QUERYINTERFACE(IClientCompilationSink,Guid,dwQueryVer);
...
bool CClientKernel::OnProcessControlData(WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize, HWND hWndSend)
{
	...
	case IPC_CMD_GF_PROPERTY_INFO: //道具消息
		{
...
VOID CClientKernel::OnUserItemUpdate(IClientUserItem * pIClientUserItem, tagUserStatus & LastStatus)
{
	...
	if(m_pIStringMessage)
	{
		...
		{
			TCHAR strMsg[64]={0};
			_sntprintf(strMsg,CountArray(strMsg),TEXT("%s 断线回来了！"), pIClientUserItem->GetUserInfo()->szNickName);
...
bool CClientKernel::OnProcessMainConfig(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	case IPC_SUB_GF_GROWLEVEL_CONFIG: //等级配置
		{
			//变量定义
			IPC_GF_GrowLevelConfig * pGrowLevlConfig=(IPC_GF_GrowLevelConfig *)pData;
			WORD wHeadSize=sizeof(IPC_GF_GrowLevelConfig)-sizeof(pGrowLevlConfig->GrowLevelItem);

			//效验参数
			ASSERT((wHeadSize+pGrowLevlConfig->wLevelCount*sizeof(pGrowLevlConfig->GrowLevelItem[0]))==wDataSize);
			if ((wHeadSize+pGrowLevlConfig->wLevelCount*sizeof(pGrowLevlConfig->GrowLevelItem[0]))!=wDataSize) return false;

			//获取对象
			CUserItemElement * pUserItemElement=CUserItemElement::GetInstance();
			pUserItemElement->SetGrowLevelConfig(pGrowLevlConfig->wLevelCount,pGrowLevlConfig->GrowLevelItem);
	...
	case IPC_SUB_GF_MATCH_INFO:		//比赛信息
		{
			//效验参数
			ASSERT(wDataSize==sizeof(IPC_GF_MatchInfo));
			if (wDataSize!=sizeof(IPC_GF_MatchInfo)) return false;

			//变量定义
			IPC_GF_MatchInfo * pMatchInfo=(IPC_GF_MatchInfo *)pData;
			//比赛类型
			m_ServerAttribute.cbMatchType=(pMatchInfo->GameMatch).cbMatchType;
			return true;
		}
	case IPC_SUB_GF_LIST_PROPERTY_TYPE:	//道具类型
		{
			//效验参数
			ASSERT(wDataSize%sizeof(tagPropertyTypeItem)==0);
			if (wDataSize%sizeof(tagPropertyTypeItem)!=0) return false;

			//变量定义
			WORD wItemCount=wDataSize/sizeof(tagPropertyTypeItem);
			tagPropertyTypeItem * ptagGamePropertyTypeItem=(tagPropertyTypeItem *)(pData);
			...
			{
				pGamePropertyListData->InsertGamePropertyTypeItem(ptagGamePropertyTypeItem++);
			}

			return true;
		}
	case IPC_SUB_GF_LIST_PROPERTY_RELAT:	//道具关系
		{
			//效验参数
			ASSERT(wDataSize%sizeof(tagPropertyRelatItem)==0);
			if (wDataSize%sizeof(tagPropertyRelatItem)!=0) return false;

			//变量定义
			WORD wItemCount=wDataSize/sizeof(tagPropertyRelatItem);
			tagPropertyRelatItem * ptagGamePropertyRelatItem=(tagPropertyRelatItem *)(pData);
			...
			for (WORD i=0;i<wItemCount;i++)
			{
				pGamePropertyListData->InsertGamePropertyRelatItem(ptagGamePropertyRelatItem++);
			}
			return true;
		}
	case IPC_SUB_GF_LIST_PROPERTY:	//道具列表
		{
			//效验参数
			ASSERT(wDataSize%sizeof(tagPropertyItem)==0);
			if (wDataSize%sizeof(tagPropertyItem)!=0) return false;

			//变量定义
			WORD wItemCount=wDataSize/sizeof(tagPropertyItem);
			tagPropertyItem * ptagGamePropertyItem=(tagPropertyItem *)(pData);
			...
			for (WORD i=0;i<wItemCount;i++)
			{
				pGamePropertyListData->InsertGamePropertyItem(ptagGamePropertyItem++);
			}
	...
	case IPC_SUB_GF_USER_ENTER:		//用户进入
		{
			...
			//激活用户
			m_GameUserManagerModule->ActiveUserItem(UserInfo,CustomFaceInfo,pIPCUserInfo->cbCompanion,szUserNote,pUserInfoHead->bIsAndroid);
...
bool CClientKernel::OnProcessMainPropertyInfo(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	switch (wSubCmdID)
	{
	case IPC_SUB_GF_PROPERTY_SUCCESS:	//道具成功
		{
			//效验参数
	...
	case IPC_SUB_GF_PROPERTY_FAILURE:	//道具失败
		{
			//效验参数
	...
	case IPC_SUB_GR_PROPERTY_MESSAGE:   //道具消息
		{
			//效验参数
	...
	case IPC_SUB_GR_PROPERTY_TRUMPET:  //道具喇叭
		{
			//效验参数
	...
	case IPC_SUB_GF_USER_TRUMPET:  //道具喇叭
		{
			//效验参数
			ASSERT(wDataSize==sizeof(IPC_GF_Trumpet_Number));
			if (wDataSize!=sizeof(IPC_GF_Trumpet_Number)) return false;

			//提取数据
			IPC_GF_Trumpet_Number * pPropertyTrumpet=(IPC_GF_Trumpet_Number *)pData;
			ASSERT(pPropertyTrumpet!=NULL);
			if(pPropertyTrumpet==NULL) return true;

			if (m_pIClientPropertySink!=NULL)
			{
				m_pIClientPropertySink->SetTrumpetNum(pPropertyTrumpet->dwTrumpetCount,pPropertyTrumpet->dwTyphonCount);
			}
...
VOID CClientKernel::SetGameFrameTitle()
{
	...
	{
		if (CServerRule::IsAllowAvertCheatMode(m_ServerAttribute.dwServerRule)==false&&m_UserAttribute.wTableID!=INVALID_TABLE)
		{
...
bool CClientKernel::PlayChatShortSound(BYTE cbGender, BYTE cbIndex)
{
	...
	if((!pGlobalUnits->m_bMuteStatuts) && pGlobalUnits->m_bAllowSound)
	{
		...
		CFile file;
		if(file.Open(szFileName2,CFile::modeRead))
		{
			bGameConfig = true;
			file.Close();
		}
