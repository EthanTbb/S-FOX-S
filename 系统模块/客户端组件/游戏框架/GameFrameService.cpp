...
{
	//发送关闭
	if(AfxGetMainWnd()!=NULL)AfxGetMainWnd()->PostMessage(WM_CLOSE);
...
bool CGameFrameService::OnEventSocket(WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	if(wMainCmdID==MDM_GF_FRAME)
	{
		...
		case SUB_GR_USER_WAIT_DISTRIBUTE:   //等待提示
			{
				//更新界面
				m_pGameFrameView->m_bShowWaitDistribute=true;
				m_pGameFrameView->RedrawWindow();
...
{
	//设置变量
	if(m_pIClientKernel->GetMeUserItem()==pIClientUserItem && pIClientUserItem->GetUserStatus()>=US_SIT)
	{
		m_pGameFrameView->m_bShowWaitDistribute=false;
	}
