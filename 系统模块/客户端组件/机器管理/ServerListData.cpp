...
bool CServerListData::InsertGameType(tagGameType * pGameType)
{
	...
	if (bInsert==true)
	{
		...
		//界面更新
		if(m_pIServerListDataSink!=NULL) m_pIServerListDataSink->OnGameItemInsert(pGameTypeItem);
	}
	else
	{
		//界面更新
		if(m_pIServerListDataSink!=NULL) m_pIServerListDataSink->OnGameItemUpdate(pGameTypeItem);
	}
...
bool CServerListData::InsertGameKind(tagGameKind * pGameKind)
{
	...
	if (bInsert==true)
	{
		...
		//插入子项
		if(m_pIServerListDataSink!=NULL) m_pIServerListDataSink->OnGameItemInsert(pGameKindItem);
	}
	else
	{
		//更新子项
...
bool CServerListData::InsertGameNode(tagGameNode * pGameNode)
{
	...
	if (bInsert==true)
	{
		...
		//插入子项
		if(m_pIServerListDataSink!=NULL) m_pIServerListDataSink->OnGameItemInsert(pGameNodeItem);
	}
	else 
	{
		//更新子项
		if(m_pIServerListDataSink!=NULL) m_pIServerListDataSink->OnGameItemUpdate(pGameNodeItem);
	}
...
bool CServerListData::InsertGamePage(tagGamePage * pGamePage)
{
	...
	if (bInsert==true)
	{
		...
		//插入子项
		if(m_pIServerListDataSink!=NULL) m_pIServerListDataSink->OnGameItemInsert(pGamePageItem);
	}
	else 
	{
		//更新子项
		if(m_pIServerListDataSink!=NULL) m_pIServerListDataSink->OnGameItemUpdate(pGamePageItem);
	}
...
bool CServerListData::InsertGameServer(tagGameServer * pGameServer)
{
	...
	//查找类型
	if(pGameServerItem->m_pGameKindItem)
	{
		//变量定义
		DWORD dwGameKindOnline=0;
	...
	if (bInsert==true)
	{
		...
		//插入子项
		if(m_pIServerListDataSink!=NULL) m_pIServerListDataSink->OnGameItemInsert(pGameServerItem);
	}
	else 
	{
		//更新子项
...
//获取总在线人数
DWORD CServerListData::GetAllOnLineCount()
{
	//定义变量
	DWORD dwAllOnLineCount=0;
	POSITION Position = NULL;

	//遍历
	do
	{
		CGameKindItem *pGameKindItem = EmunGameKindItem(Position);
		if(pGameKindItem)
		{
