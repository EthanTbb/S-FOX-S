...
CPngImage			CUserListControl::m_ImageUserStatus;				//状态位图
CImageList			CUserListControl::m_ImageListStatus;				//状态位图
...
CUserListControl::CUserListControl()
{
	...
	m_ColorUserList.crAndroidTX=COLOR_ANDROID_TX;
	m_ColorUserList.crAndroidBK=COLOR_ANDROID_BK;
...
VOID CUserListControl::PreSubclassWindow()
{
	...
	//加载状态
	if (m_ImageListStatus.m_hImageList == NULL)
	{
		//加载图片
		CBitmap Image;
		BITMAP ImageInfo;
		...
		m_ImageListStatus.Create(USER_STATUS_CX,ImageInfo.bmHeight,ILC_COLOR16|ILC_MASK,0,0);

		//插入图片
		m_ImageListStatus.Add(&Image,RGB(255,0,255));		
	}

	//加载图片
	if(m_ImageUserStatus.IsNull() == true)
	{
		m_ImageUserStatus.LoadImage(GetModuleHandle(SHARE_CONTROL_DLL_NAME),TEXT("USER_STATUS_IMAGE"));
	}

	//设置控件
	SetBkColor(m_ColorUserList.crNormalBK);
	SetImageList(&m_ImageListStatus,LVSIL_SMALL);
...
INT CUserListControl::SortItemData(LPARAM lParam1, LPARAM lParam2, WORD wColumnIndex, bool bAscendSort)
{
	...
	case UD_USER_MEDAL:		//用户元宝
		{
			//获取变量
			SCORE lUserIngot1=pIClientUserItem1->GetUserIngot();
			SCORE lUserIngot2=pIClientUserItem2->GetUserIngot();

			//对比数据
			INT nResult=lUserIngot1>lUserIngot2?SORT_AFTER:SORT_FRONT;
			return (bAscendSort==true)?nResult:-nResult;
		}
...
VOID CUserListControl::ConstructString(VOID * pItemData, WORD wColumnIndex, LPTSTR pszString, WORD wMaxCount)
{
	...
	case UD_USER_MEDAL:		//用户元宝
		{
			_sntprintf(pszString,wMaxCount,TEXT("%ld"),pIClientUserItem->GetUserIngot());
			break;
		}
	case UD_EXPERIENCE:		//用户经验
		{
	...
	case UD_WIN_RATE:		//用户胜率
		{
			...
			_sntprintf(pszString,wMaxCount,TEXT("%4.2f%%"),fWinRate);
			break;
		}
	case UD_LOST_RATE:		//用户输率
		{
			FLOAT fLostRate=pIClientUserItem->GetUserLostRate();
			_sntprintf(pszString,wMaxCount,TEXT("%4.2f%%"),fLostRate);
			break;
		}
	case UD_DRAW_RATE:		//用户和率
		{
			FLOAT fDrawRate=pIClientUserItem->GetUserDrawRate();
			_sntprintf(pszString,wMaxCount,TEXT("%4.2f%%"),fDrawRate);
			break;
		}
	case UD_FLEE_RATE:		//用户逃率
		{
			FLOAT fFleeRate=pIClientUserItem->GetUserFleeRate();
			_sntprintf(pszString,wMaxCount,TEXT("%4.2f%%"),fFleeRate);
			break;
		}
	case UD_GAME_LEVEL:		//游戏等级
		{
			...
			//获取等级
			if (CServerRule::IsAllowAvertCheatMode(m_dwServerRule) && (pIClientUserItem->GetUserID()!=m_dwMySelfUserID))
				_sntprintf(pszString,wMaxCount,TEXT("-"));
			else
...
VOID CUserListControl::GetItemColor(LPDRAWITEMSTRUCT lpDrawItemStruct, COLORREF&crColorText, COLORREF&crColorBack)
{
	...
	else if(pIClientUserItem->IsAndroidUser()==true)
	{
		//机器颜色
		crColorText=m_ColorUserList.crAndroidTX;
		crColorBack=m_ColorUserList.crAndroidBK;
	}
	else
	{
		//默认颜色
	...
	case UD_IMAGE_FLAG:		//用户标志
		{
			...
			m_ImageUserStatus.DrawImage(pDC,rcSubItem.left,rcSubItem.top,USER_STATUS_CX,nStatusHeight,uImageIndex*USER_STATUS_CX,0,USER_STATUS_CX,nStatusHeight);
	...
	case UD_IMAGE_STATUS:	//用户状态
		{
			...
			if(cbGender > GENDER_MANKIND) cbGender=GENDER_MANKIND;
	...
	case UD_EXPERIENCE:  	//用户经验
		{
			...
			pUserItemElement->DrawExperience(pDC,rcSubItem,dwExperience);
...
//设置接口
bool CUserListControl::SetServerRule(DWORD dwServerRule)
{
