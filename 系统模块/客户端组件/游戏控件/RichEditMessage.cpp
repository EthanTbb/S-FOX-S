...
bool CRichEditMessage::InsertUserOffLine(LPCTSTR pszUserName)
{
	...
	//插入事件
	InsertUserAccounts(pszUserName);
	InsertString(TEXT("断线了，请耐心等候！"),COLOR_WARN);
...
{
	return InsertNormalString(pszString,COLOR_NORMAL);
}

//系统消息
bool CRichEditMessage::InsertSystemString(LPCTSTR pszString)
{
	return InsertSystemString(pszString,COLOR_SYSTEM_STRING);
}

//提示消息
bool CRichEditMessage::InsertPromptString(LPCTSTR pszString)
{
	return InsertPromptString(pszString,COLOR_SYSTEM_STRING);
}

//公告消息
bool CRichEditMessage::InsertAfficheString(LPCTSTR pszString)
{
	return InsertAfficheString(pszString,COLOR_SYSTEM_STRING);
}
...
//用户喇叭
bool CRichEditMessage::InsertUserTrumpet(LPCTSTR pszSendUser,LPCTSTR pszString,COLORREF crColor)
{
	//插入换行
	LONG lTextLength=GetWindowTextLength();
	if (lTextLength!=0L) InsertString(TEXT("\r\n"),COLOR_EVENT);
	...
	//插入消息
	InsertUserAccounts(pszSendUser);
	InsertString(TEXT("发小喇叭 "),COLOR_EVENT);

	//加载图片
	if (m_ImageDataTrumpet.GetBitmap()==NULL)
	{
		HINSTANCE hInstance=GetModuleHandle(SHARE_CONTROL_DLL_NAME);
		LoadRichEditImage(hInstance,IDB_IMAGE_TRUMPET,m_ImageDataTrumpet);
	}
	//插入数据
...
//普通消息
bool CRichEditMessage::InsertNormalString(LPCTSTR pszString,COLORREF crColor)
{
	//插入换行
	LONG lTextLength=GetWindowTextLength();
	if (lTextLength!=0L) InsertString(TEXT("\r\n"),COLOR_EVENT);
...
//系统消息
bool CRichEditMessage::InsertSystemString(LPCTSTR pszString,COLORREF crColor)
{
	//插入换行
	LONG lTextLength=GetWindowTextLength();
	if (lTextLength!=0L) InsertString(TEXT("\r\n"),COLOR_EVENT);
...
//提示消息
bool CRichEditMessage::InsertPromptString(LPCTSTR pszString,COLORREF crColor)
{
	//插入换行
	LONG lTextLength=GetWindowTextLength();
	if (lTextLength!=0L) InsertString(TEXT("\r\n"),COLOR_EVENT);
...
//公告消息
bool CRichEditMessage::InsertAfficheString(LPCTSTR pszString,COLORREF crColor)
{
	//插入换行
	LONG lTextLength=GetWindowTextLength();
	if (lTextLength!=0L) InsertString(TEXT("\r\n"),COLOR_EVENT);
...
bool CRichEditMessage::InsertWhisperChat(LPCTSTR pszSendUser, LPCTSTR pszString, COLORREF crColor, bool bMyselfString)
{
	...
	return InsertWhisperChat(pszSendUser, pszString, crColor, bMyselfString, SystemTime);
}

//用户私聊
bool CRichEditMessage::InsertWhisperChat(LPCTSTR pszSendUser, LPCTSTR pszString, COLORREF crColor, bool bMyselfString, const SYSTEMTIME &time)
{
	//保存选择
	CHARRANGE CharRange;
	GetSel(CharRange);
