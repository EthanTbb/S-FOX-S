...
VOID CFlashActiveX::SetPosition(INT nXPos, INT nYPos, INT nWidth, INT nHeight, UINT nFlags)
{
	...
	//设置位置
	SetWindowPos(m_hWnd,NULL,nXPos,nYPos,nWidth,nHeight,nFlags|SWP_NOZORDER|SWP_NOZORDER);
...
BOOL CFlashActiveX::CreateControl(DWORD dwStyle, CWnd * pParentWnd, CRect rcCreate, HINSTANCE hInstance, IUnknownEx * pIUnknownEx)
{
	...
	//设置组件
	m_pIFalshControl->PutWMode(TEXT("transparent"));

	//显示控件
	if (FAILED(m_pIOleObject->DoVerb(OLEIVERB_UIACTIVATE,NULL,(IOleClientSite *)this,0L,NULL,NULL)))
	{
		ASSERT(FALSE);
		return FALSE;
	}
...
VOID CFlashActiveX::PlayMovie()
{
	...
	m_pIFalshControl->PutLoop(VARIANT_FALSE);
...
VOID CFlashControl::CreateControl(CWnd * pParentWnd, CRect rcCreate, IUnknownEx * pIUnknownEx)
{
	...
	m_FlashContainer.CreateControl(WS_POPUP|WS_CLIPSIBLINGS,pParentWnd,rcCreate,hInstance,pIUnknownEx);
	return;
}

//可视控制
VOID CFlashControl::ShowControl(WORD wVisible)
{
	m_FlashContainer.ShowControl(wVisible);
