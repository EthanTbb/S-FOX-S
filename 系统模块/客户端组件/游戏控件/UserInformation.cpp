...
#define INFORMATION_FILE			L"Information.DAT"					//文件名字
...
bool CUserInformation::LoadInformation()
{
	...
	try
	{
		...
		_snwprintf(szInfomationFile,CountArray(szInfomationFile),L"%s\\UserInformation\\%d_%s",(LPCWSTR)(CT2CW(szDirectory)),pGlobalUserData->dwUserID,INFORMATION_FILE);
...
tagCompanionInfo * CUserInformation::InsertCompanionInfo(IClientUserItem * pIClientUserItem, BYTE cbCompanion)
{
	...
	if (pCompanionInfo==NULL)
	{
		...
		pCompanionInfo->cbFaceID=(BYTE)pIClientUserItem->GetFaceID();
	...
	try
	{
		...
		//文件目录
		TCHAR szUserInformationForder[MAX_PATH]=TEXT("");
		_sntprintf(szUserInformationForder,CountArray(szUserInformationForder),TEXT("%s\\UserInformation"),(LPCWSTR)(CT2CW(szDirectory)));
		if (!PathIsDirectory(szUserInformationForder))
		{
			::CreateDirectory(szUserInformationForder, NULL);
		}
