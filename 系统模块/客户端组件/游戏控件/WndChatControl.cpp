...
BOOL CWndChatControl::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	{
		//选择用户
		INT_PTR nIndex=LOWORD(wParam)-IDM_HISTORY_CHAT_USER;

		//选择用户
		ASSERT(nIndex>=0 && nIndex<m_ChatUserItemArray.GetCount());
...
VOID CWndChatControl::OnSize(UINT nType, INT cx, INT cy)
{
	...
	if (m_btControl.m_hWnd!=NULL)
	{
		...
		m_btControl.SetWindowPos(NULL,0,cy-rcButton.Height(),0,0,SWP_NOSIZE|SWP_NOZORDER);
	}
	...
	if (m_btClose.m_hWnd!=NULL)
	{
		...
		m_btClose.SetWindowPos(NULL,cx-rcButton.Width(),cy-rcButton.Height(),0,0,SWP_NOSIZE|SWP_NOZORDER);
	}
