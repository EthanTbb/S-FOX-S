...
//用户信息
#define USER_INFO_CX					180								//信息宽度
#define USER_INFO_CY					186								//信息高度
#define CHAT_INPUT_CY					100								//输入高度
#define TOOL_BAR_CY						30								//面板高度	
...
BEGIN_MESSAGE_MAP(CDlgWhisper, CFGuiDialog)
	...
	//控件命令
	ON_CBN_SELCHANGE(IDC_COMBO_ANDROID, OnCbnSelchangeComboxAndroid)
...
//构造函数
CDlgWhisper::CDlgWhisper() : CFGuiDialog(IDD_DLG_WHISPER)
{
	//状态变量
	m_bLableRound=false;
	m_bCreateFlag=false;
	...
	//界面控件	
	m_pLabelRound=NULL;
	m_pLabelToolBar=NULL;
	m_pLabelChatArea=NULL;
	m_pLabelUserInfoArea=NULL;
	m_pLabelFunctionArea=NULL;
	m_pLabelChatHistory=NULL;
	m_pLabelHistoryArea=NULL;

	//界面控件
	m_pbtCancel=NULL;
	m_pbtSendChat=NULL;
	m_pbtUserMenu=NULL;
	m_pbtSendControl=NULL;
	m_pbtChatButton1=NULL;
	m_pbtChatButton2=NULL;
	m_pbtChatButton3=NULL;

	//界面控件
	m_pbtChatHistory=NULL;
	m_pbtAddFriend=NULL;
	m_pbtSetDetest=NULL;
	m_pbtMessageSetting=NULL;
	m_pbtShieldThisChat=NULL;
...
VOID CDlgWhisper::DoDataExchange(CDataExchange * pDX)
{
	...
	//控制按钮
	DDX_Control(pDX, IDC_COMBO_ANDROID, m_ComboAndroidList);	
...
BOOL CDlgWhisper::OnInitDialog()
{
	...
	SetIcon(LoadIcon(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDI_WHISPER)),FALSE);
...
//初始控件
void CDlgWhisper::InitControlUI()
{
	...
	//聊天记录
	m_pLabelChatHistory = CLabelUI::Create(&m_PaintManager,pParent,TEXT("")); 
	m_pLabelChatHistory->SetBkImage(TEXT("file='IMAGE_ROUND_FRAME' restype='PNG' corner='6,6,6,6'"));

	//记录区域	
	m_pLabelHistoryArea = CLabelUI::Create(&m_PaintManager,pParent,TEXT("")); 
	m_pLabelHistoryArea->SetBkImage(TEXT("file='WHISPER_AREA_ROUND' restype='PNG' corner='4,4,4,4'"));

	//聊天环绕
	m_pLabelChatArea = CLabelUI::Create(&m_PaintManager,pParent,TEXT("")); 
	m_pLabelChatArea->SetBkImage(TEXT("file='WHISPER_AREA_ROUND' restype='PNG' corner='4,4,4,4'"));

	//信息环绕
	m_pLabelUserInfoArea = CLabelUI::Create(&m_PaintManager,pParent,TEXT("")); 
	m_pLabelUserInfoArea->SetBkImage(TEXT("file='WHISPER_AREA_ROUND' restype='PNG' corner='4,4,4,4'"));

	//功能环绕
	m_pLabelFunctionArea = CLabelUI::Create(&m_PaintManager,pParent,TEXT("")); 
	m_pLabelFunctionArea->SetBkImage(TEXT("file='WHISPER_AREA_ROUND' restype='PNG' corner='4,4,4,4'"));

	//工具面板
	m_pLabelToolBar = CLabelUI::Create(&m_PaintManager,pParent,TEXT("")); 
	m_pLabelToolBar->SetBkColor(ARGB(255,209,197,174));

	//关闭控件
	m_pbtCancel = CButtonUI::Create(&m_PaintManager,pParent,TEXT(""));
	m_pbtCancel->SetStatusImage(TEXT("file='BT_WHISPER_CLOSE' restype='PNG'"));

	//发送按钮
	m_pbtSendChat = CButtonUI::Create(&m_PaintManager,pParent,TEXT(""));
	m_pbtSendChat->SetStatusImage(TEXT("file='BT_WHISPER_SEND' restype='PNG'"));

	//控制按钮
	m_pbtSendControl = CButtonUI::Create(&m_PaintManager,pParent,TEXT(""));
	m_pbtSendControl->SetStatusImage(TEXT("file='BT_WHISPER_DOWN' restype='PNG'"));

	//聊天按钮
	m_pbtChatButton1 = CButtonUI::Create(&m_PaintManager,pParent,TEXT(""));
	m_pbtChatButton1->SetStatusImage(TEXT("file='BT_CHAT_BUTTON_1' restype='PNG'"));
	m_pbtChatButton1->SetFadeOut(false);

	//聊天按钮
	m_pbtChatButton2 = CButtonUI::Create(&m_PaintManager,pParent,TEXT(""));
	m_pbtChatButton2->SetStatusImage(TEXT("file='BT_CHAT_BUTTON_2' restype='PNG'"));
	m_pbtChatButton2->SetFadeOut(false);

	//聊天按钮
	m_pbtChatButton3 = CButtonUI::Create(&m_PaintManager,pParent,TEXT(""));
	m_pbtChatButton3->SetStatusImage(TEXT("file='BT_CHAT_BUTTON_3' restype='PNG'"));
	m_pbtChatButton3->SetFadeOut(false);

	//用户菜单
	m_pbtUserMenu = CButtonUI::Create(&m_PaintManager,pParent,TEXT(""));
	m_pbtUserMenu->SetStatusImage(TEXT("file='BT_USER_MENU' restype='PNG'"));
	m_pbtUserMenu->SetFadeOut(false);
	m_pbtUserMenu->SetVisible(false);

	//聊天记录
	m_pbtChatHistory = CButtonUI::Create(&m_PaintManager,pParent,TEXT(""));
	m_pbtChatHistory->SetStatusImage(TEXT("file='BT_CHAT_HISTORY' restype='PNG'"));
	m_pbtChatHistory->SetFadeOut(false);

	//功能按钮
	m_pbtAddFriend = CButtonUI::Create(&m_PaintManager,pParent,TEXT(""));
	m_pbtAddFriend->SetStatusImage(TEXT("file='BT_ADD_FRIEND_1' restype='PNG'"));

	//功能按钮
	m_pbtSetDetest = CButtonUI::Create(&m_PaintManager,pParent,TEXT(""));	
	m_pbtSetDetest->SetStatusImage(TEXT("file='BT_SET_DETEST_1' restype='PNG'"));

	//功能按钮
	m_pbtShieldThisChat = CButtonUI::Create(&m_PaintManager,pParent,TEXT(""));
	m_pbtShieldThisChat->SetStatusImage(TEXT("file='BT_SHIELD_CHAT_1' restype='PNG'"));

	//消息设置
	m_pbtMessageSetting = CButtonUI::Create(&m_PaintManager,pParent,TEXT(""));
	m_pbtMessageSetting->SetStatusImage(TEXT("file='BT_MESSAGE_SETTING' restype='PNG'"));
...
//消息提醒
void CDlgWhisper::Notify(TNotifyUI &  msg)
{
	__super::Notify(msg);
	...
	if (lstrcmp(msg.sType, TEXT("click")) == 0)
	{
		...
		if(pControlUI==m_pbtCancel) 
		{
			//关闭聊天
			return OnCancel();
		}
		else if(pControlUI==m_pbtSendChat) 
		{
			//发送聊天
			return OnBnClickedSendChat();
		}
		else if(pControlUI==m_pbtSendControl) 
		{
			//发送控制
			return OnBnClickedSendControl();
		}
		else if(pControlUI==m_pbtUserMenu) 
		{
			//用户菜单
			return OnBnClickedUserMenu();
		}
		else if(pControlUI==m_pbtChatButton1) 
		{
			//聊天按钮
			return OnBnClickedChatButton1();
		}
		else if(pControlUI==m_pbtChatButton2) 
		{
			//聊天按钮
			return OnBnClickedChatButton2();
		}
		else if(pControlUI==m_pbtChatButton3) 
		{
			//聊天按钮
			return OnBnClickedChatButton3();
		}
		else if(pControlUI==m_pbtChatHistory)
		{
			//聊天记录
			return OnBnClickedChatHistory();
		}
		else if(pControlUI==m_pbtAddFriend)
		{
			//添加好友
			return OnBnClickAddFriend();
		}
		else if(pControlUI==m_pbtSetDetest)
		{
			//设为厌恶
			return OnBnClickSetDetest();
		}
		else if(pControlUI==m_pbtMessageSetting)
		{
			//消息设置
			return OnBnClickMessageSetting();
		}
		else if(pControlUI==m_pbtShieldThisChat)
		{
			//屏蔽聊天
			return OnBnClickedShieldThisChat();
		}
	}
}

////绘画标题
//VOID CDlgWhisper::OnDrawSystemTitle(CDC * pDC, INT nWdith, INT nHeight)
//{
//	//资源句柄
...
//	//加载标题
//	CPngImage ImageLogo;
//	ImageLogo.LoadImage(hInstance,TEXT("IMAGE_WHISPER_TITLE"));
//
//	//绘制标题
//	ImageLogo.DrawImage(pDC,12,10);
//}

//绘画消息
VOID CDlgWhisper::DrawClientAreaEnd(CDC * pDC, INT nWidth, INT nHeight)
{
	//变量定义
	CRect rcLabelRound(m_pLabelRound->GetPos());

	//区域膨胀
	rcLabelRound.InflateRect(-4,-4,-4,-4);

	//框架位置
	INT nTBorder=rcLabelRound.top;
	INT nBBorder=nHeight-rcLabelRound.bottom;
	INT nLBorder=rcLabelRound.left;
	INT nRBorder=nWidth-rcLabelRound.right;
	...
	//控制区域
	CRect rcChatControl;
	rcChatControl.right=nWidth-nRBorder-5;
	rcChatControl.bottom=nHeight-nBBorder-9;
...
void CDlgWhisper::OnEndPaintWindow(HDC hDC)
{
	::SelectObject(hDC,m_PaintManager.GetFont(-1));

	CDC *pDC = CDC::FromHandle(hDC);
	CRect rcWindow;
	::GetWindowRect(GetSafeHwnd(),&rcWindow);
	DrawClientAreaEnd(pDC, rcWindow.Width(), rcWindow.Height());
}
...
bool CDlgWhisper::EfficacyUserChat(LPCTSTR pszChatString, WORD wExpressionIndex)
{
	...
	if (CUserRight::CanWisper(m_dwUserRight)==false && bAllNotMaster )
	{
		...
		Information.ShowMessageBox(TEXT("很抱歉，您没有发送私聊信息的权限！"));
	...
	if( CServerRule::IsForfendWisperChat(dwServerRule) && bAllNotMaster )
	{
		//if( CServerRule::IsMemberCanChat(m_dwServerRule) == false || m_pIMyselfUserItem->GetMemberOrder() == 0 )
		{
			//提示信息
			CDlgInformation Information(this);
			Information.ShowMessageBox(TEXT("抱歉，此游戏房间不允许用户私聊！"));
			return false;
		}
	}
		...
		!CUserRight::CanGameChat(m_dwUserRight))
	{
		//提示信息
		CDlgInformation Information(this);
		Information.ShowMessageBox(TEXT("抱歉，此游戏房间不允许在游戏中与同桌的玩家私聊！"));
		return false;
	}
...
//创建窗口
bool CDlgWhisper::CreateWhisperWnd(DWORD dwConversationID,DWORD dwSendUserID,IClientUserItem * pIClientUserItem[], WORD wUserCount, CWnd * pParentWnd)
{
	//设置变量
	m_ClientUserItemArray.RemoveAll();

	//设置变量
	m_dwSendUserID=dwSendUserID;
		...
		wOppositeCount == 1 && dwOppositeGameID != 0L )
	{
		...
		if(cbSourCompanion==CP_FRIEND)
			m_pbtAddFriend->SetStatusImage(TEXT("file='BT_ADD_FRIEND_2' restype='PNG'"));
		else
			m_pbtAddFriend->SetStatusImage(TEXT("file='BT_ADD_FRIEND_1' restype='PNG'"));

		if(cbSourCompanion==CP_DETEST)
			m_pbtSetDetest->SetStatusImage(TEXT("file='BT_SET_DETEST_2' restype='PNG'"));
		else
			m_pbtSetDetest->SetStatusImage(TEXT("file='BT_SET_DETEST_1' restype='PNG'"));

		if(cbSourCompanion==CP_SHIELD)
			m_pbtShieldThisChat->SetStatusImage(TEXT("file='BT_SHIELD_CHAT_2' restype='PNG'"));
		else
			m_pbtShieldThisChat->SetStatusImage(TEXT("file='BT_SHIELD_CHAT_1' restype='PNG'"));
	}
...
VOID CDlgWhisper::RectifyControl(INT nWidth, INT nHeight)
{
	...
	//变量定义
	CSize SizeRoundCorner(4,4);

	//背景环绕
	CRect rcPos(10,44,nWidth-10,nHeight-12);
	m_pLabelRound->SetPos(rcPos);
	m_pLabelRound->SetFixedWidth(rcPos.Width());
	m_pLabelRound->SetFixedHeight(rcPos.Height());	

	//聊天环绕
	CRect rcChat(CRect(rcPos.left+6,rcPos.top+8,rcPos.right-USER_INFO_CX-16,rcPos.bottom-8));
	m_pLabelChatArea->SetFixedWidth(rcChat.Width());
	m_pLabelChatArea->SetFixedHeight(rcChat.Height());
	m_pLabelChatArea->SetPos(rcChat);

	//信息环绕
	CRect rcUserInfo(rcPos.right-USER_INFO_CX-8,rcPos.top+8,rcPos.right-8,rcPos.top+USER_INFO_CY+8);
	m_pLabelUserInfoArea->SetFixedWidth(rcUserInfo.Width());
	m_pLabelUserInfoArea->SetFixedHeight(rcUserInfo.Height());
	m_pLabelUserInfoArea->SetPos(rcUserInfo);

	//功能环绕
	CRect rcFunction(rcPos.right-USER_INFO_CX-8,rcPos.top+USER_INFO_CY+12,rcPos.right-8,rcPos.bottom-8);
	m_pLabelFunctionArea->SetFixedWidth(rcFunction.Width());
	m_pLabelFunctionArea->SetFixedHeight(rcFunction.Height());
	m_pLabelFunctionArea->SetPos(rcFunction);	

	//输入位置
	CRect rcChatInput=rcChat;
	rcChatInput.InflateRect(-SizeRoundCorner.cx,-rcChatInput.Height()+114,-SizeRoundCorner.cx,-36);

	//信息位置
	CRect rcChatMessage=rcChat;
	rcChatMessage.InflateRect(-SizeRoundCorner.cx,-SizeRoundCorner.cy,-SizeRoundCorner.cy,-(rcChat.bottom-rcChatInput.top+TOOL_BAR_CY+2));

	//工具面板
	CRect rcToolBar(rcChatInput.left-2,rcChatInput.top-TOOL_BAR_CY-2,rcChatInput.right+2,rcChatInput.top-2);	
	m_pLabelToolBar->SetFixedWidth(rcToolBar.Width());
	m_pLabelToolBar->SetFixedHeight(rcToolBar.Height());
	m_pLabelToolBar->SetPos(rcToolBar);

	//变量定义
	CSize SizeCancel(m_pbtCancel->GetFixedWidth(),m_pbtCancel->GetFixedHeight());
	CSize SizeSendChat(m_pbtSendChat->GetFixedWidth(),m_pbtSendChat->GetFixedHeight());
	CSize SizeSendControl(m_pbtSendControl->GetFixedWidth(),m_pbtSendControl->GetFixedHeight());

	//控制按钮
	m_pbtSendControl->SetFixedXY(CPoint(rcChat.right-SizeSendControl.cx-6,rcChat.bottom-SizeSendControl.cy-4));
	m_pbtSendChat->SetFixedXY(CPoint(rcChat.right-SizeSendControl.cx-SizeSendChat.cx-8,rcChat.bottom-SizeSendChat.cy-4));
	m_pbtCancel->SetFixedXY(CPoint(rcChat.right-SizeSendControl.cx-SizeSendChat.cx-SizeCancel.cx-12,rcChat.bottom-SizeCancel.cy-4));

	//变量定义
	CSize SizeChatButton1(m_pbtChatButton1->GetFixedWidth(),m_pbtChatButton1->GetFixedHeight());
	CSize SizeChatHistory(m_pbtChatHistory->GetFixedWidth(),m_pbtChatHistory->GetFixedHeight());

	//聊天按钮
	const int offset_y = 7;
	m_pbtChatButton1->SetFixedXY(CPoint(rcChat.left+SizeRoundCorner.cx+4,rcToolBar.top+offset_y));
	m_pbtChatButton2->SetFixedXY(CPoint(rcChat.left+SizeRoundCorner.cx+4+(SizeChatButton1.cx+4)*1,rcToolBar.top+offset_y));
	m_pbtChatButton3->SetFixedXY(CPoint(rcChat.left+SizeRoundCorner.cx+4+(SizeChatButton1.cx+4)*2,rcToolBar.top+offset_y));
	m_pbtChatHistory->SetFixedXY(CPoint(rcChat.right-SizeRoundCorner.cx-SizeChatHistory.cx,rcToolBar.top+offset_y));

	//用户菜单
	m_pbtUserMenu->SetFixedXY(CPoint(rcUserInfo.left+68,rcUserInfo.top+8));

	//变量定义
	CSize SizeAddFriend(m_pbtAddFriend->GetFixedWidth(),m_pbtAddFriend->GetFixedHeight());

	//功能按钮
	m_pbtAddFriend->SetFixedXY(CPoint(rcFunction.left+8,rcFunction.top+6));
	m_pbtSetDetest->SetFixedXY(CPoint(rcFunction.left+8,rcFunction.top+6+(SizeAddFriend.cy+4)*1));
	m_pbtShieldThisChat->SetFixedXY(CPoint(rcFunction.left+8,rcFunction.top+6+(SizeAddFriend.cy+4)*2));
	m_pbtMessageSetting->SetFixedXY(CPoint(rcFunction.left+8,rcFunction.top+6+(SizeAddFriend.cy+4)*3));	

	//历史记录
	CRect rcHistory(rcPos.left,nHeight,nWidth - rcPos.left,nHeight + NORMAL_HISTORY_HEGIHT - 10);
	m_pLabelChatHistory->SetPos(rcHistory);
	m_pLabelChatHistory->SetFixedWidth(rcHistory.Width());
	m_pLabelChatHistory->SetFixedHeight(rcHistory.Height());

	//记录区域
	rcHistory.InflateRect(-SizeRoundCorner.cx,-SizeRoundCorner.cy,-SizeRoundCorner.cx,-SizeRoundCorner.cy);
	m_pLabelHistoryArea->SetPos(rcHistory);
	m_pLabelHistoryArea->SetFixedWidth(rcHistory.Width());
	m_pLabelHistoryArea->SetFixedHeight(rcHistory.Height());
	...
	//移动控件
	DeferWindowPos(hDwp,m_ChatInput,NULL,rcChatInput.left,rcChatInput.top,rcChatInput.Width(),rcChatInput.Height(),uFlags);
	DeferWindowPos(hDwp,m_ChatMessage,NULL,rcChatMessage.left,rcChatMessage.top,rcChatMessage.Width(),rcChatMessage.Height(),uFlags);
	DeferWindowPos(hDwp,m_HistoryEdit,NULL,rcHistory.left+SizeRoundCorner.cx,rcHistory.top+SizeRoundCorner.cy,rcHistory.Width()-SizeRoundCorner.cx*2,rcHistory.Height()-SizeRoundCorner.cy*2,uFlags);

	//控制按钮
	CRect rcComboxList;
	m_ComboAndroidList.GetWindowRect(rcComboxList);
	DeferWindowPos(hDwp,m_ComboAndroidList,NULL,rcChatInput.right-m_pbtSendControl->GetFixedWidth()-m_pbtCancel->GetFixedWidth()*2-rcComboxList.Width()-20,nHeight-m_pbtCancel->GetFixedHeight()-21,0,0,uFlags|SWP_NOSIZE);
...
VOID CDlgWhisper::DrawUserInfoArea(CDC * pDC, INT nXPos, INT nYPos, INT nWidth, INT nHeight)
{
	...
	//加载资源
	CPngImage ImageFaceFrame;
	ImageFaceFrame.LoadImage(GetModuleHandle(PLATFORM_RESOURCE_DLL_NAME),TEXT("IMAGE_FACE_BACK"));	

	//头像框架
	ImageFaceFrame.DrawImage(pDC,nXPos+6,nYPos+7);
	...
	if (m_ClientUserItemArray.GetCount()>=2L)
	{
		...
		CRect rcGameScore(nXPos+11,nYPos+95,nXPos+nWidth-5,nYPos+95+12);
		CRect rcUserStatus(nXPos+11,nYPos+116,nXPos+nWidth-5,nYPos+116+12);
		CRect rcUnderWrite(nXPos+11,nYPos+137,nXPos+nWidth-5,nYPos+137+12);
		...
		//用户等级
		//_sntprintf(szString,CountArray(szString),TEXT("等级：%d 级"),wExperienceLevel);
...
bool CDlgWhisper::SendExpressionPacket(IClientUserItem * pIClientUserItem, WORD wItemIndex)
{
	...
	WisperExpression.dwSendUserID=m_dwSendUserID;
...
bool CDlgWhisper::SendWhisperChatPacket(IClientUserItem * pIClientUserItem, LPCTSTR pszChatString)
{
	...
	//构造数据
	WisperChat.dwSendUserID=m_dwSendUserID;
...
bool CDlgWhisper::SendColloquyChatPacket(IClientUserItem * pIClientUserItem[], WORD wUserCount, LPCTSTR pszChatString)
{
	...
	ColloquyChat.dwSendUserID=m_dwSendUserID;
...
VOID CDlgWhisper::OnBnClickedSendControl()
{
	...
	//获取位置
	CRect rcButton = m_pbtSendControl->GetPos();
...
{
	//创建表情
	CRect rcButton = m_pbtChatButton1->GetPos();
	ClientToScreen(&rcButton);
...
VOID CDlgWhisper::OnBnClickedChatButton2()
{
	...
	//弹出菜单
	CRect rcButton = m_pbtChatButton2->GetPos();
	ClientToScreen(&rcButton);
...
//加载机器
VOID CDlgWhisper::LoadAndroidList()
{
	//权限判断
	if (!(CMasterRight::CanManagerAndroid(m_dwMasterRight)==true))
	{
		//设置控件
		m_ComboAndroidList.EnableWindow(FALSE);
		m_ComboAndroidList.ShowWindow(SW_HIDE);
	...
	do
	{
		pIClientUserItem=m_pIPlazaUserManager->EnumUserItem(wEnumIndex++);
		if(pIClientUserItem!=NULL)
		{
			if(pIClientUserItem->IsAndroidUser()==true)
			{
				nIndex=m_ComboAndroidList.AddString(pIClientUserItem->GetNickName());
				m_ComboAndroidList.SetItemData(nIndex,pIClientUserItem->GetUserID());
				if(pIClientUserItem->GetUserID()==m_dwSendUserID) 
				{
					m_ComboAndroidList.SetCurSel(nIndex);
				}
			}
		}
	...
	//更新数据
	m_ComboAndroidList.UpdateData();
	m_ComboAndroidList.ShowWindow(SW_SHOW);
}
...
//选择变更
VOID CDlgWhisper::OnCbnSelchangeComboxAndroid()
{
	//选择判断
	if(m_ComboAndroidList.GetCurSel()==-1) return;
	
	//获取数据
	INT nIndex = m_ComboAndroidList.GetCurSel();
	m_dwSendUserID = static_cast<DWORD>(m_ComboAndroidList.GetItemData(nIndex));
