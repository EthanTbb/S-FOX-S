...
#define TIME_AVTIVE_MISSION				15000L							//激活任务
...
bool CMissionList::OnEventMissionRead(TCP_Command Command, VOID * pData, WORD wDataSize)
{
	if (Command.wMainCmdID==MDM_GP_SERVER_LIST)
	{
		...
		case SUB_GP_AGENT_KIND:
			{
				...
				//变量定义
				WORD wItemCount=wDataSize/sizeof(tagAgentGameKind);
				tagAgentGameKind * pAgentGameKind=(tagAgentGameKind *)(pData);
				...
				for (WORD i=0;i<wItemCount;i++)
				{
					pServerListData->InsertAgentGameKind(pAgentGameKind++);
				}
		...
		case SUB_GP_MATCH_FINISH:	//比赛完成
			{
				...
				//完成通知
				pServerListData->OnEventMatchFinish();

				return true;
			}
		case SUB_GP_VIDEO_OPTION:	//视频配置
			{
				//效验数据
				ASSERT(wDataSize==sizeof(tagAVServerOption));
				if(wDataSize!=sizeof(tagAVServerOption)) return false;

				//变量定义
				tagAVServerOption *pAVServerOption=(tagAVServerOption*)pData;

				//获取对像
				CPlatformFrame *pPlatfromFrame=CPlatformFrame::GetInstance();
				if(pPlatfromFrame==NULL) return false;
		...
		case SUB_GP_LIST_PROPERTY_TYPE:	//道具类型
			{
				//效验参数
		...
		case SUB_GP_LIST_PROPERTY_RELAT:	//道具关系
			{
				//效验参数
		...
		case SUB_GP_LIST_PROPERTY:	//道具列表
			{
				//效验参数
		...
		case SUB_GP_LIST_PROPERTY_SUB:	//道具列表
			{
				//效验参数
				ASSERT(wDataSize%sizeof(tagPropertySubItem)==0);
				if (wDataSize%sizeof(tagPropertySubItem)!=0) return false;

				//变量定义
				WORD wItemCount=wDataSize/sizeof(tagPropertySubItem);
				tagPropertySubItem * ptagGamePropertySubItem=(tagPropertySubItem *)(pData);
				...
				for (WORD i=0;i<wItemCount;i++)
				{
					pGamePropertyListData->InsertGamePropertySubItem(ptagGamePropertySubItem++);
				}
