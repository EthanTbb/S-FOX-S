...
CServerListView::CServerListView()
{
	...
	m_LastServerItem.m_GameType.wSortID=80;
	m_LastServerItem.m_GameType.wJoinID=0;
	_sntprintf(m_LastServerItem.m_GameType.szTypeName, CountArray(m_LastServerItem.m_GameType.szTypeName), TEXT("最近常登录房间"));
	...
	case ItemGenre_Page:	//定制子项
		{
			...
			if (pParentListItem!=NULL)
			{
				//常规插入
				HTREEITEM hTreeItemNormal=pParentListItem->m_hTreeItemNormal;
				if (hTreeItemNormal!=NULL) pGameListItem->m_hTreeItemNormal=InsertGameListItem(pszDisplayName,IND_BROWSE,pGameListItem,hTreeItemNormal);

				//喜爱插入
				HTREEITEM hTreeItemAttach=pParentListItem->m_hTreeItemAttach;
				if (hTreeItemAttach!=NULL) pGameListItem->m_hTreeItemAttach=InsertGameListItem(pszDisplayName,IND_BROWSE,pGameListItem,hTreeItemAttach);
			}
			else
			{
				//常规插入
