...
public:
	...
	//道具信息
	bool SendGamePropertyTypeInfo();
	//发送关系
	bool SendGamePropertyRelatInfo();
	//发送道具
	bool SendGamePropertyInfo();
	//道具数量
	bool SendGamePropertyTrumpet(DWORD dwTrumpetCount,DWORD dwTyphonCount);
	//等级配置
	bool SendGrowLevelConfig(tagGrowLevelConfig GrowLevelItem[],WORD wLevelCount);
	//房间信息
	...
	//比赛信息
	bool SendMatchInfo(tagGameMatch& GameMatch);
...
public:
	...
	//用户属性
	bool SendUserAttrib(DWORD dwUserID, tagUserAttrib & UserAttrib);
	//用户头像
