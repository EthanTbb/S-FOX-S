...
bool CCustomFaceManager::LoadUserCustomFace(DWORD dwUserID, DWORD dwCustomID, tagCustomFaceInfo96 & CustomFaceInfo)
{
	...
	//目标目录
	TCHAR szCustomFacePath[MAX_PATH]=TEXT("");
	_sntprintf(szCustomFacePath,CountArray(szCustomFacePath),TEXT("%s\\CustomFace\\%ld_96.DAT"),szWorkDirectory,dwUserID);
...
bool CCustomFaceManager::PerformDownLoad(DWORD dwUserID, DWORD dwCustomID)
{
	...
	_sntprintf(szCustomFace,CountArray(szCustomFace),TEXT("%s/CustomFace.aspx?UserID=%ld&CustomID=%ld"),pGlobalWebLink->GetPlatformLink(),dwUserID,dwCustomID);
