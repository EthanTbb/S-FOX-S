...
#ifndef _DEBUG
	_sntprintf(szTitle,CountArray(szTitle),TEXT("[ %s ] -- [ %s ] -- [ %s ]"),m_ModuleInitParameter.GameServiceAttrib.szGameName,pszServerName,pszStatusName);
#else
...
//获取连接
bool CGameServerDlg::GetPlatformDBParameter(tagDataBaseParameter & DataBaseParameter)
{
	//获取路径
...
VOID CGameServerDlg::OnBnClickedOptionServer()
{
	...
	DlgServerWizard.SetPersonalRoomParameter(m_ModuleInitParameter.PersonalRoomOption, m_ModuleInitParameter.m_pCreateRoomRightAndFee);
...
VOID CGameServerDlg::OnBnClickedOptionMatch()
{
	...
	if (DlgServerMatch.OpenGameMatch(m_ModuleInitParameter.GameServiceAttrib,m_ModuleInitParameter.GameServiceOption)) 
	{
		//配置比赛
		m_ModuleInitParameter.GameMatchOption=DlgServerMatch.m_GameMatchOption;
		...
		_sntprintf(szMessage,CountArray(szMessage),TEXT("[%s] 比赛配置加载成功"),m_ModuleInitParameter.GameMatchOption.szMatchName);
