...
bool CLockTimeMatch::VerifyUserEnterMatch(IServerUserItem * pIServerUserItem)
{
	...
	if (m_pLockTimeMatch->lMatchCullScore>0 && pIServerUserItem->GetUserScore()<m_pLockTimeMatch->lMatchCullScore)
	{
		_sntprintf(szMessage,CountArray(szMessage),TEXT("由于您的比赛币低于 %lld，您将不能继续游戏！"),m_pLockTimeMatch->lMatchCullScore);
...
bool CLockTimeMatch::SendReviveNotify(IServerUserItem * pIServerUserItem, BYTE cbReviveTimesed, bool bUseSafeCard)
{
	if (bUseSafeCard==false)
	{
		...
		//构造消息
		_sntprintf(MatchRevive.szNotifyContent,CountArray(MatchRevive.szNotifyContent),TEXT("您的比赛币低于 %lld，将不能继续游戏。 您只需支付 %I64d 金币便可复活继续比赛，您确定要复活吗？"),m_pLockTimeMatch->lMatchCullScore,m_pLockTimeMatch->lReviveFee);
	...
	else
	{
		//变量定义
		TCHAR szMessage[128];
		_sntprintf(szMessage,CountArray(szMessage),TEXT("您的比赛币低于 %lld，将不能继续游戏。由于您购买了保险卡，系统将免费为您复活一次！"),m_pLockTimeMatch->lMatchCullScore);
...
bool CLockTimeMatch::SendSafeCardNotify(IServerUserItem * pIServerUserItem)
{
	...
	//构造消息
	_sntprintf(MatchSafeCard.szNotifyContent,CountArray(MatchSafeCard.szNotifyContent),TEXT("恭喜您报名成功！本场比赛出售保险卡, 被淘汰可免费复活一次，售价 %I64d 金币，您是否要购买？"),MatchSafeCard.lSafeCardFee);
...
bool CLockTimeMatch::SendDeductFeeNotify(IServerUserItem * pIServerUserItem)
{
	...
	if (m_pMatchOption->cbDeductArea==DEDUCT_AREA_SERVER && MatchFee.lSignupFee>0)
	{
		...
		_sntprintf(MatchFee.szNotifyContent,CountArray(MatchFee.szNotifyContent),TEXT("参赛将扣除报名费 %I64d %s，确认要参赛吗？"),m_pMatchOption->lSignupFee,pszFeeType[m_pMatchOption->cbFeeType-FEE_TYPE_GOLD]);
	}
...
void CLockTimeMatch::SendTableUserMatchInfo(ITableFrame *pITableFrame, WORD wChairID)
{
	...
	_sntprintf(MatchInfo.szTitle[1], CountArray(MatchInfo.szTitle[1]),TEXT("游戏基数：%lld"), m_pLockTimeMatch->lMatchInitScore);
