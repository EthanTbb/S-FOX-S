	...
	try
	{
		...
		if(lResultCode == DB_SUCCESS)
		{
			...
			while (PlatformDBModule->IsRecordsetEnd()==false)
			{
				...
				personalRoomOptionRightAndFee.stCreateRoomPayFee[iCount].dwPayFee = PlatformDBAide.GetValue_LONGLONG(TEXT("TableFee"));
				personalRoomOptionRightAndFee.stCreateRoomPayFee[iCount].dwIniScore = PlatformDBAide.GetValue_LONGLONG(TEXT("IniScore"));
				iCount++;
				//移动记录
				PlatformDBModule->MoveToNext();
			}
