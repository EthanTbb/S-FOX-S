	...
	try
	{
		...
		if (GameMatchDBAide.ExecuteProcess(TEXT("GSP_GS_LoadMatchReward"),true)==DB_SUCCESS)
		{
			...
			while (GameMatchDBModule->IsRecordsetEnd()==false)
			{
				...
				MatchRewardInfo.lRewardGold=GameMatchDBAide.GetValue_LONGLONG(TEXT("RewardGold"));
				MatchRewardInfo.lRewardIngot=GameMatchDBAide.GetValue_LONGLONG(TEXT("RewardIngot"));
...
bool CMatchInfoManager::ReadGameMatchOption(CDataBaseAide & GameMatchDBAide, tagGameMatchOption & GameMatchResult)
{
	...
	GameMatchResult.lSignupFee= GameMatchDBAide.GetValue_LONGLONG(TEXT("SignupFee"));
